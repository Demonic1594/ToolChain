...
void SafeLoadMonIconPalette(SpeciesEnum species) {
    ...
    if (SpeciesHas8bppSprites(species)) {
        LoadPalette(gSabrevoir8bppIconPal + (SABREVOIR_8BPP_ICON_PAL_OFFSET - 0x100),
                    SABREVOIR_8BPP_ICON_PAL_OFFSET,
                    SABREVOIR_8BPP_ICON_PAL_COLORS * 2);
        return;
    }
...
const u8 *GetMonIconTiles4bpp(SpeciesEnum species, u32 personality) {
    // classic 4bpp tiles, used by systems with fixed 16-tile icon slots (PC storage)
    const u8 *iconSprite = gMonIconTable[species];
    if (SpeciesHasGenderDifference(species) && GetGenderFromSpeciesAndPersonality(species, personality) == MON_FEMALE) {
        iconSprite = gMonIconTableFemale[species];
    }
    ...
    if (sprite->animDelayCounter == 0) {
        ...
        switch (frame) {
            ...
            case -2:
                ...
                break;
            default: {
                u32 frameSize = sSpriteImageSizes[sprite->oam.shape][sprite->oam.size];
                if (sprite->oam.bpp == ST_OAM_8BPP)
                    frameSize *= 2;
                RequestSpriteCopy(
                    // pointer arithmetic is needed to get the correct pointer to perform the sprite copy on.
                    // because sprite->images is a struct def, it has to be casted to (u8 *) before any
                    // arithmetic can be performed.
                    (u8 *)sprite->images + (frameSize * frame),
...
static u8 CreateMonIconSprite(struct MonIconSpriteTemplate *iconTemplate, s16 x, s16 y, u8 subpriority) {
    u8 spriteId;
    bool32 is8bpp = (iconTemplate->image == gSabrevoir8bppIcon);

    struct SpriteFrameImage image = {NULL, sSpriteImageSizes[iconTemplate->oam->shape][iconTemplate->oam->size]};
    if (is8bpp)
        image.size *= 2; // double tile allocation + VRAM copy size for 8bpp
...
void sub_80D328C(struct Sprite *sprite) {
    ...
    if (sprite->oam.bpp == ST_OAM_8BPP)
        image.size *= 2; // free the doubled tile allocation too
    sprite->images = &image;
