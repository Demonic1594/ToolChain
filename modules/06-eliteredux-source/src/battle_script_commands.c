...
static void Cmd_ppreduce(void) {
    ...
    if (gBattleMons[gBattlerAttacker].pp[gCurrMovePos]) {
        ...
        // (Cmd_ppreduce runs once per move turn, so no Parental Bond exception is needed here)
...
s32 CalcCritChanceStage(u8 battlerAtk, u8 battlerDef, MoveEnum move, u16 typeEffectiveness) {
    ...
    critChance += ((gBattleMoves[move].flags & FLAG_HIGH_CRIT) != 0) + (holdEffectAtk == HOLD_EFFECT_SCOPE_LENS) +
                  2 * (holdEffectAtk == HOLD_EFFECT_LUCKY_PUNCH && (GET_BASE_SPECIES_ID(gBattleMons[battlerAtk].species) == SPECIES_HAPPINY ||
                                                                    GET_BASE_SPECIES_ID(gBattleMons[battlerAtk].species) == SPECIES_CHANSEY ||
                                                                    GET_BASE_SPECIES_ID(gBattleMons[battlerAtk].species) == SPECIES_HAPPINY_REDUX ||
                                                                    GET_BASE_SPECIES_ID(gBattleMons[battlerAtk].species) == SPECIES_CHANSEY_REDUX ||
                                                                    GET_BASE_SPECIES_ID(gBattleMons[battlerAtk].species) == SPECIES_BLISSEY_REDUX ||
                                                                    GET_BASE_SPECIES_ID(gBattleMons[battlerAtk].species) == SPECIES_BLISSEY)) +
...
static void Cmd_adjustdamage(void) {
    ...
    if (holdEffect == HOLD_EFFECT_FOCUS_BAND && (Random() % 100) < param) {
        ...
        if (BATTLER_HAS_ABILITY(gBattlerTarget, ABILITY_STURDY) && BATTLER_MAX_HP(gBattlerTarget))
            ...
            // Unlike Sturdy, Absolute Guard is not gated behind BATTLER_MAX_HP: it must
            // clamp every lethal hit to 1 HP, not just the first hit of the turn, so
            // multi-hit moves (Bullet Seed, Dragon Darts, etc.) can't punch through it
            // after the first strike drops the target below full HP.
    ...
    SetMoveEffect(primary, certain);
            ...
            switch (gBattleScripting.moveEffect) {
                ...
                case MOVE_EFFECT_LEECH_SEED:
                    ...
                    REQUIRE_NOT(IS_BATTLER_OF_TYPE(gBattlerTarget, TYPE_GRASS) && !IsMyceliumMightActive(gBattlerAttacker))
                    ...
                    *(&gBattleStruct->sentInPokes) >>= 1;
                    ...
                    if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_HP) &&
                        ...
                        if (holdEffect == HOLD_EFFECT_TRAINING_BAND) {
                            ...
                            if ((s32)GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL) < (s32)highestLevel - 4) {
...
static void TryCheekPouch(u32 battlerId, u32 itemId) {
    AbilityEnum ability;
    if (ItemId_GetPocket(itemId) == POCKET_BERRIES && ((ability = BattlerHasAbility(battlerId, ABILITY_GLUTTONY, FALSE)) || (ability = BattlerHasAbility(battlerId, ABILITY_BEAST_OF_GLUTTONY, FALSE))) && CanBattlerHeal(battlerId) &&
...
static void Cmd_hpthresholds(void) {
    ...
    if (!(gBattleTypeFlags & BATTLE_TYPE_DOUBLE)) {
        ...
        result = gBattleMons[opposingBank].hp * 100 / max(1, gBattleMons[opposingBank].maxHP);
...
static void Cmd_hpthresholds2(void) {
    ...
    if (!(gBattleTypeFlags & BATTLE_TYPE_DOUBLE)) {
        ...
        result = (hpSwitchout - gBattleMons[opposingBank].hp) * 100 / max(1, hpSwitchout);
...
static bool32 ClearDefogHazards(u8 battlerAtk, bool32 clear) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (GetBattlerSide(battlerAtk) != i) {
            ...
            DEFOG_CLEAR(SIDE_STATUS_SMOKESCREEN, smokescreenTimer, BattleScript_SideStatusWoreOffReturn, MOVE_SMOKESCREEN);
        }
...
static void Cmd_various(void) {
    ...
    switch (cmd) {
            ...
            if (GetBattlerTurnOrderNum(gBattlerAttacker) > GetBattlerTurnOrderNum(gBattlerTarget))
                ...
                switch (move) {
                    ...
                    case MOVE_BEAK_BLAST:
                        ...
                        break;
                    default:
                        if (gQueuedAttackCount >= MAX_QUEUED_EXTRA_ATTACKS) {
                            gBattlescriptCurrInstr = ptr;
                            break;
                        }
        ...
        case VARIOUS_TRY_INSTRUCT:
            ptr = READ_PTR_INC;
            if (gLastMoves[gActiveBattler] == 0 || gLastMoves[gActiveBattler] == 0xFFFF || gLastMoves[gActiveBattler] == MOVE_STRUGGLE ||
                gLastMoves[gActiveBattler] == MOVE_KINGS_SHIELD) {
                gBattlescriptCurrInstr = ptr;
            } else {
                ...
                if (sMoveEffectsForbiddenToInstruct[i] != FORBIDDEN_INSTRUCT_END)
                    gBattlescriptCurrInstr = ptr;
                else {
                    for (i = 0; i < MAX_MON_MOVES; i++) {
                    ...
                    if (i >= MAX_MON_MOVES || gBattleMons[gActiveBattler].pp[i] == 0 || gQueuedAttackCount >= MAX_QUEUED_EXTRA_ATTACKS)
                        gBattlescriptCurrInstr = ptr;
                    else {
                        gQueuedExtraAttackData[++gQueuedAttackCount] = (struct ExtraAttackActionStruct){
        ...
        case VARIOUS_PSYCHO_SHIFT:
            ...
            if (gBattleMons[gBattlerAttacker].status1 & STATUS1_PARALYSIS && !(gBattleMons[gBattlerTarget].status1 & STATUS1_PARALYSIS)) {
                ...
                if (!CanBePoisoned(gBattlerAttacker, gBattlerTarget, gCurrentMove)) {
                    BattleScriptPush(ptr);
        ...
        case VARIOUS_SWAP_WITH:
            ...
            }
            {
                ...
                if ((gSideStatuses[GET_BATTLER_SIDE(gActiveBattler)] ^ gSideStatuses[GET_BATTLER_SIDE(gBattlerAttacker)]) & SIDE_STATUS_FUTUREATTACK) {
                    ...
                    gSideStatuses[GET_BATTLER_SIDE(gBattlerAttacker)] ^= SIDE_STATUS_FUTUREATTACK;
                }
...
static void Cmd_trychoosesleeptalkmove(void) {
    ...
    {
        gBattlescriptCurrInstr += 5;
    } else if (gQueuedAttackCount >= MAX_QUEUED_EXTRA_ATTACKS) {
        gBattlescriptCurrInstr += 5;
        ...
        for (i = 0; i < PARTY_SIZE; i++) {
            ...
            if (species != SPECIES_NONE && species != SPECIES_EGG) {
                ...
                if (gBattlerPartyIndexes[gBattlerAttacker] == i) {
                    healMon = !IsSoundproof(gBattlerAttacker);
                } else if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE && gBattlerPartyIndexes[BATTLE_PARTNER(gBattlerAttacker)] == i) {
                    healMon = !IsSoundproof(BATTLE_PARTNER(gBattlerAttacker));
                } else {
    ...
    if (gMoveResultFlags & MOVE_RESULT_NO_EFFECT) {
        ...
        if (gVolatileStructs[gBattlerAttacker].furyCutterCounter != 5 &&
            !(gTurnStructs[gBattlerAttacker].parentalBondInitialCount > 0 &&
              gTurnStructs[gBattlerAttacker].parentalBondOn < gTurnStructs[gBattlerAttacker].parentalBondInitialCount))  // Don't increment counter on second hit
...
static void Cmd_handleballthrow(void) {
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_TRAINER) {
        ...
        } else  // mon may be caught, calculate shakes
        {
            ...
            if (gLastUsedItem == ITEM_MASTER_BALL) {
                shakes = maxShakes;
            } else if (odds == 0) {  // 0 catch odds: the ball can never shake, let alone catch (also avoids a division by zero)
                shakes = 0;
            } else {
