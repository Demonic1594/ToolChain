...
bool8 TryRunFromBattle(u8 battler) {
    ...
    if (holdEffect == HOLD_EFFECT_SHED_SHELL) {
        ...
        if (InBattlePyramid()) {
            ...
            speedVar = (gBattleMons[battler].speed * pyramidMultiplier) / max(1, gBattleMons[BATTLE_OPPOSITE(battler)].speed) + (gBattleStruct->runTries * 30);
        ...
        if (InBattlePyramid()) {
            pyramidMultiplier = GetPyramidRunMultiplier();
            speedVar = (gBattleMons[battler].speed * pyramidMultiplier) / max(1, gBattleMons[runningFromBattler].speed) + (gBattleStruct->runTries * 30);
        ...
        else  // Debug roms can always run
        {
            ...
            speedVar = (gBattleMons[battler].speed * 128) / max(1, gBattleMons[runningFromBattler].speed) + (gBattleStruct->runTries * 30);
            if (speedVar > (Random() & 0xFF)) effect++;
...
void CancelMultiTurnMoves(u8 battler) {
    ...
    for (i = 0; i < gBattlersCount; i++) {
        FILTER(gVolatileStructs[i].skyDropped && gVolatileStructs[i].skyDroppedBy == battler)
    ...
    do {
        ...
        switch (gBattleStruct->turnCountersTracker) {
            ...
            case ENDTURN_MAGIC_ROOM:
                if (gFieldStatuses & STATUS_FIELD_MAGIC_ROOM && !gFieldTimers.started.magicRoom && --gFieldTimers.magicRoomTimer == 0) {
    ...
    while (gBattleStruct->turnEffectsBattlerId < gBattlersCount && gBattleStruct->turnEffectsTracker <= ENDTURN_BATTLER_COUNT) {
        ...
        switch (gBattleStruct->turnEffectsTracker) {
            ...
            case ENDTURN_TOXIC_WASTE_DAMAGE:
                if (getMonotypeChampType() == TYPE_POISON && GET_BATTLER_SIDE(gActiveBattler) == B_SIDE_PLAYER)
            ...
            case ENDTURN_WRAP:  // wrap
                if (getMonotypeChampType() == TYPE_BUG && gBattleMons[gActiveBattler].hp != 0 && GET_BATTLER_SIDE(gActiveBattler) == B_SIDE_PLAYER) {
                    ...
                    } else  // broke free
                    {
                        ...
                        gVolatileStructs[gActiveBattler].wrapAbility = ABILITY_NONE;
                    }
                    BattleScriptExecute(gBattlescriptCurrInstr);
                    effect++;
                }
            ...
            case ENDTURN_COILED_UP: {
                u16 lastMove = gLastMoves[gActiveBattler];
                bool32 validLastMove = lastMove != MOVE_NONE && lastMove != 0xFFFF;
                if ((gStatuses4[gActiveBattler] & STATUS4_COILED) && validLastMove && (gBattleMoves[lastMove].flags & FLAG_STRONG_JAW_BOOST) &&
                ...
                if (gStatuses4[gActiveBattler] & STATUS4_CUTTHROAT && validLastMove &&
                    IsKeenEdge(gActiveBattler, lastMove, GetTypeBeforeUsingMove(lastMove, gActiveBattler)))
...
bool8 UseOutOfTurnAttack(u8 battler, u8 target, AbilityEnum ability, MoveEnum move, u8 movePower) {
    if (gQueuedAttackCount >= MAX_QUEUED_EXTRA_ATTACKS) return FALSE;
...
bool32 ShouldChangeFormHpBased(u32 battler) {
    ...
    for (i = 0; i < ARRAY_COUNT(gHpTransformations); i++) {
        if (BattlerHasAbility(battler, gHpTransformations[i].ability, FALSE)) {
            ...
            if (species == gHpTransformations[i].lowHpSpecies && gBattleMons[battler].hp > gBattleMons[battler].maxHP / gHpTransformations[i].hpFraction) {
                ...
                UpdateAbilityStateIndicesForNewSpecies(battler, gHpTransformations[i].highHpSpecies);
    ...
    if (species == SPECIES_DARMANITAN && (BattlerHasAbility(battler, ABILITY_ZEN_MODE, FALSE)) && gBattleMons[battler].hp != 0) {
        ...
        UpdateAbilityStateIndicesForNewSpecies(battler, SPECIES_DARMANITAN_ZEN_MODE);
    ...
    if (species == SPECIES_DARMANITAN_GALARIAN && (BattlerHasAbility(battler, ABILITY_ZEN_MODE, FALSE)) && gBattleMons[battler].hp != 0) {
        gBattleScripting.abilityPopupOverwrite = ABILITY_ZEN_MODE;
        gBattlerAttacker = battler;
        UpdateAbilityStateIndicesForNewSpecies(battler, SPECIES_DARMANITAN_ZEN_MODE_GALARIAN);
    ...
    if (gBattleMons[battler].species == SPECIES_CHERRIM && BattlerHasAbility(battler, ABILITY_FLOWER_GIFT, FALSE) && gBattleMons[battler].hp != 0) {
        if (gBattleWeather & (WEATHER_SUN_ANY)) {
            ...
            gBattlerAttacker = battler;
            UpdateAbilityStateIndicesForNewSpecies(battler, SPECIES_CHERRIM_SUNSHINE);
        ...
        if (!(gBattleWeather & (WEATHER_SUN_ANY))) {
            gBattleScripting.abilityPopupOverwrite = ABILITY_FLOWER_GIFT;
            gBattlerAttacker = battler;
            UpdateAbilityStateIndicesForNewSpecies(battler, SPECIES_CHERRIM);
...
bool8 UseEntryMove(u8 battler, AbilityEnum ability, u16 extraMove, u8 movePower) {
    ...
    if (target < gBattlersCount && gQueuedAttackCount < MAX_QUEUED_EXTRA_ATTACKS && CanUseExtraMove(battler, target)) {
        gQueuedExtraAttackData[++gQueuedAttackCount] = (struct ExtraAttackActionStruct){
            .ability = ability,
            .attacker = battler,
            .move = extraMove,
...
bool32 HasEnoughHpToEatBerry(u32 battlerId, u32 hpFraction, u32 itemId) {
    ...
    if (hpFraction <= 4 && (BattlerHasAbility(battlerId, ABILITY_GLUTTONY, FALSE) || BattlerHasAbility(battlerId, ABILITY_BEAST_OF_GLUTTONY, FALSE)) && isBerry && gBattleMons[battlerId].hp <= gBattleMons[battlerId].maxHP / 2) {
        return TRUE;
    }
...
static u16 CalcMoveBasePower(MoveEnum move, u8 battlerAtk, u8 battlerDef) {
    ...
    switch (gBattleMoves[move].effect) {
        case EFFECT_FLING: {
            u16 item = gTurnStructs[battlerAtk].flungItem ? gTurnStructs[battlerAtk].flungItem : gBattleMons[battlerAtk].item;
            switch (item) {
                case ITEM_IRON_BALL:
...
#undef CHECK_WEATHER_DOUBLE_BOOST
    ...
    switch (GetBattlerHoldEffect(battlerAtk, TRUE)) {
        ...
        case HOLD_EFFECT_EXPERT_BELT:
            if (typeEffectivenessModifier >= GetSuperEffectiveThreshold()) MulModifier(&finalModifier, UQ_4_12(1.2));
            break;
        case HOLD_EFFECT_LIFE_ORB:
    ...
    switch (GetBattlerHoldEffect(battlerDef, TRUE)) {
        ...
        case HOLD_EFFECT_RESIST_BERRY:
            if (moveType == ItemId_GetSecondaryId(gBattleMons[battlerDef].item) && (moveType == TYPE_NORMAL || typeEffectivenessModifier >= GetSuperEffectiveThreshold()) &&
    ...
    if (typeEffectivenessModifier >= GetSuperEffectiveThreshold() && gBattleMoves[move].effect == EFFECT_MISC_HIT &&
...
s32 DoMoveDamageCalcInternal(
    ...
    {
        u32 defStat = CalcDefenseStat(move, battlerAtk, battlerDef, moveType, gIsCriticalHit, updateFlags);
        dmg /= max(1, defStat);  // a defense stat of 0 would divide by zero
    }
    dmg = (dmg / 50) + 2;
    ...
    switch (getMonotypeChampType()) {
        case TYPE_GRASS:
            ...
            if (GetBattlerSide(battlerDef) != B_SIDE_PLAYER) dmg = dmg * 7 / 10;
            break;
        case TYPE_PSYCHIC:
        ...
        case TYPE_GROUND:
            ...
            if (GetBattlerSide(battlerDef) != B_SIDE_PLAYER) {
                if (typeEffectivenessModifier >= GetSuperEffectiveThreshold()) dmg = dmg * 13 / 20;
            }
            break;
        case TYPE_FIGHTING:
            // Super effective moves do neutral damage
            // (divides out the actual modifier so it also works for the hell mode chart: 1.5x / 3.5x)
            if (typeEffectivenessModifier > UQ_4_12(1.0))
                dmg = dmg * UQ_4_12(1.0) / typeEffectivenessModifier;
...
// Hell mode: super-effective damage is reduced. Original values restored: 2x -> 1.5x and 4x -> 2.5x
// (the original comment claimed 4x -> 2x, but the code has always used 2.5x). Three-type 8x results are not reduced.
static u16 ApplyHellModeTypeChart(u16 modifier) {
...
// Smallest type-effectiveness modifier that counts as "super effective" (2x normally, 1.5x in hell mode).
// Use this instead of comparing against UQ_4_12(2.0), which never matches in hell mode.
u16 GetSuperEffectiveThreshold(void) { return (isHellMode() && HELL_MODE_TYPE_EFFECTIVENESS_CHANGE) ? UQ_4_12(1.5) : UQ_4_12(2.0); }
...
static u16 CalcTypeEffectivenessMultiplierInternal(MoveEnum move, Type moveType, u8 battlerAtk, u8 battlerDef, bool32 recordAbilities, u16 modifier) {
    ...
    modifier = ApplyHellModeTypeChart(modifier);
...
u16 CalcPartyMonTypeEffectivenessMultiplier(MoveEnum move, SpeciesEnum speciesDef, u16 abilityDef, u8 battlerDef) {
    ...
    if (move != MOVE_STRUGGLE && IsValidType(moveType)) {
        ...
        modifier = ApplyHellModeTypeChart(modifier);
        if (abilityDef == ABILITY_ABSOLUTE_GUARD && modifier <= UQ_4_12(1.0) && gBattleMoves[move].power)
            modifier = UQ_4_12(0.0);
    }
...
AbilityEnum IsUnnerveAbilityOnOpposingSide(u8 battlerId) {
    ...
    if (IsBattlerAlive(opponent)) {
        RETURN_ABILITY_IF_FLAG(opponent, FALSE, unnerve)
    }
    return FALSE;
}
