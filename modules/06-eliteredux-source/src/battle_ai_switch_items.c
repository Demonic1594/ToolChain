...
static u32 CalculateHazardDamage(void)
{
    ...
    if ((gSideTimers[GetBattlerSide(gActiveBattler)].spikesAmount > 0) && IsBattlerGrounded(gActiveBattler))
    {
...
static u8 PredictFoesMoveType(u32 opposingBattler)
{
    ...
    if (defType1 != defType2)
        typeDmg1 *= GetTypeModifier(atkType1, defType2, gActiveBattler, opposingBattler);
    ...
    if (atkType1 != atkType2)
    {
        ...
        if (defType1 != defType2)
            typeDmg2 *= GetTypeModifier(atkType2, defType2, gActiveBattler, opposingBattler);
    ...
    for (i = 0; i < AI_MOVE_HISTORY_COUNT; i++)
    {
        if (moves[i] != MOVE_NONE && gBattleMoves[moves[i]].split != SPLIT_STATUS)
        {
            ...
            if (defType1 != defType2)
                typeDmg3 *= GetTypeModifier(atkType1, defType2, gActiveBattler, opposingBattler);
    ...
    while (bits != 0x3F) // All mons were checked.
    {
        ...
        for (i = firstId; i < lastId; i++)
        {
            if (!(gBitTable[i] & invalidMons) && !(gBitTable[i] & bits))
            {
                ...
                typeDmg = ApplyModifier(GetTypeModifier(atkType1, defType1, gActiveBattler, opposingBattler), typeDmg);
                if (atkType2 != atkType1)
                    typeDmg = ApplyModifier(GetTypeModifier(atkType2, defType1, gActiveBattler, opposingBattler), typeDmg);
                if (defType2 != defType1)
                {
                    typeDmg = ApplyModifier(GetTypeModifier(atkType1, defType2, gActiveBattler, opposingBattler), typeDmg);
                    if (atkType2 != atkType1)
                        typeDmg = ApplyModifier(GetTypeModifier(atkType2, defType2, gActiveBattler, opposingBattler), typeDmg);
                }
                if (bestDmg < typeDmg)
                {
                    bestDmg = typeDmg;
