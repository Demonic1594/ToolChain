...
void CheckSingleHitKo(int baseDamage, int defenderHp, int multiplier, int procChance, int flatDamage, int setCrit, struct MoveState* moveState) {
    defenderHp = max(1, defenderHp);  // used as a divisor below
    if (multiplier) baseDamage = ApplyModifier(multiplier, baseDamage);
    if (baseDamage + flatDamage > defenderHp) {
        ...
        if (minDamage + flatDamage >= defenderHp) {
            ...
            // Clamp to 1/16 steps
            koChance = (16 * (defenderHp - minDamage) / max(1, baseDamage - minDamage) + 1) * (100 / 16);
        }
        if (procChance) {
...
int CalculateKoChanceFine(
    ...
    if (moveState->critChance > 0) {
        CheckSingleHitKo(baseDamage, defenderHp, critMultiplier, sCritChance[min(4, moveState->critChance) - 1], bonusDamage, !moveState->koChance, moveState);
    ...
    if (doubleDamageChance && moveState->critChance > 0) {
        CheckSingleHitKo(baseDamage,
                         ...
                         ApplyModifier(sCritChance[min(4, moveState->critChance) - 1], doubleDamageChance),
                         bonusDamage,
                         !moveState->koChance,
                         moveState);
    }
...
int ScoreMoveDamage(int battlerAtk,
    ...
    int defenderHp = max(1, gBattleMons[battlerDef].hp);  // used as a divisor below
    ...
    switch (gBattleMoves[move].effect) {
        ...
        case EFFECT_LEVEL_DAMAGE:
            if (CheckImmunities(battlerAtk, battlerDef, move, moveType, -1, &scoreOther, moveState)) return scoreOther;
            moveContainer->fixedDamage = TRUE;
            baseDamage = gBattleMons[battlerAtk].level;
    ...
    baseDamageAverage = max(1, baseDamageAverage);  // used as a divisor below (a zero-damage estimate is possible)
    requiredHits = 0;
    if (damageShield) {
        ...
        case MULTIHIT_TRIPLE_KICK:
            ...
            if (moveState->accuracy >= 100) goto HANDLE_KO_MULTIHIT;
            {
                ...
                if (hitsToKo == 1) {
                    ...
                    moveState->overkillInHalves = 2 * hitCount / max(1, requiredHits) - 2;  // requiredHits is 0 when there is no shield to break
                }
            }
            break;
    }
