...
void CreateMonWithEVSpread(struct Pokemon* mon, SpeciesEnum species, u8 level, u8 fixedIV, u8 evSpread) {
    ...
    evAmount = (statCount != 0) ? MAX_TOTAL_EVS / statCount : 0;
...
void GetSpeciesName(u8* name, SpeciesEnum species) {
    ...
    // Bound is '<' (not '<='): callers pass POKEMON_NAME_LENGTH + 1 byte buffers, and the
    // trailing EOS below is written at index POKEMON_NAME_LENGTH at most.
    // species >= NUM_SPECIES guards the table end (SPECIES_EGG == NUM_SPECIES - 1 is valid).
...
void PokemonToBattleMon(struct Pokemon* src, struct BattlePokemon* dst) {
    ...
    if (dst->species == SPECIES_SHEDINJA)
    {
        dst->type1 = TYPE_NORMAL;
        dst->type2 = TYPE_GHOST;
        dst->type3 = TYPE_DARK;
    }
...
// Handle ITEM1 effects (in-battle stat boosting effects)
            ...
            case 3:
                ...
                if ((itemEffect[i] & ITEM3_POISON) && HealStatusConditions(mon, partyIndex, STATUS1_POISON_ANY | STATUS1_TOXIC_COUNTER, battlerId) == 0)
                    retVal = FALSE;
    ...
    for (i = 0; i < MAX_LEVEL_UP_MOVES; i++) {
        ...
        if (moveLevel <= level) {
            if (disableLearned) {
                ...
                j = MAX_MON_MOVES;  // treat every move as not known, collect all of them
            }
...
u8 GetLevelCap(void) {
    ...
    static const u8 levelCapsStandard[] = {20, 28, 44, 55, 65, 80, 90, 100, 100, 100};  // Easy
    static const u8 levelCapsMore[] = {18, 25, 40, 50, 55, 70, 85, 92, 95, 100};        // More
    static const u8 levelCapsStrict[] = {16, 23, 36, 45, 50, 55, 60, 70, 80, 100};      // Elite
            ...
            if (battleEvent != BATTLE_EVENT_NONE) {
                switch (battleEvent) {
                    case BATTLE_EVENT_EXTRA_ABILITIES_1:
                        if (abilityNumber == 0) return GetTrainerBattleEventData(trainerId, i, BATTLE_EVENT_DATA0);
                        break;
                    case BATTLE_EVENT_EXTRA_ABILITIES_2:
                        if (abilityNumber == 1) return GetTrainerBattleEventData(trainerId, i, BATTLE_EVENT_DATA0);
                        break;
                    case BATTLE_EVENT_EXTRA_ABILITIES_3:
                        if (abilityNumber == 2) return GetTrainerBattleEventData(trainerId, i, BATTLE_EVENT_DATA0);
                        break;
                }
            }
        }
