...
static u8 getRole(SpeciesEnum species);
...
EWRAM_DATA struct ExtraAttackActionStruct gQueuedExtraAttackData[MAX_QUEUED_EXTRA_ATTACKS + 1] = {0};
...
EWRAM_DATA ItemEnum gLastUsedItem = 0;
...
EWRAM_DATA MoveEnum gLastPrintedMoves[MAX_BATTLERS_COUNT] = {0};
EWRAM_DATA MoveEnum gLastMoves[MAX_BATTLERS_COUNT] = {0};
EWRAM_DATA MoveEnum gLastChosenMove[MAX_BATTLERS_COUNT] = {0};
EWRAM_DATA MoveEnum gLastLandedMoves[MAX_BATTLERS_COUNT] = {0};
EWRAM_DATA u16 gLastHitByType[MAX_BATTLERS_COUNT] = {0};
EWRAM_DATA MoveEnum gLastResultingMoves[MAX_BATTLERS_COUNT] = {0};
EWRAM_DATA MoveEnum gLockedMoves[MAX_BATTLERS_COUNT] = {0};
EWRAM_DATA MoveEnum gLastUsedMove = 0;
EWRAM_DATA u8 gLastHitBy[MAX_BATTLERS_COUNT] = {0};
EWRAM_DATA MoveEnum gChosenMoveByBattler[MAX_BATTLERS_COUNT] = {0};
...
EWRAM_DATA Status3 gStatuses3[MAX_BATTLERS_COUNT] = {0};
EWRAM_DATA Status4 gStatuses4[MAX_BATTLERS_COUNT] = {0};
...
EWRAM_DATA WeatherFlag gBattleWeather = 0;
...
EWRAM_DATA struct BattleScripting gSavedBattleScripting = {0};
...
// rom const data
static const struct ScanlineEffectParams sIntroScanlineParams16Bit = {(void*)REG_ADDR_BG3HOFS, SCANLINE_EFFECT_DMACNT_16BIT, 1};

// unused
static const struct ScanlineEffectParams sIntroScanlineParams32Bit = {(void*)REG_ADDR_BG3HOFS, SCANLINE_EFFECT_DMACNT_32BIT, 1};
...
const u8 gTypeNames[NUMBER_OF_MON_TYPES][TYPE_NAME_LENGTH + 1] = {
    [TYPE_NORMAL] = _("Normal"), [TYPE_FIGHTING] = _("Fight"), [TYPE_FLYING] = _("Flying"), [TYPE_POISON] = _("Poison"),   [TYPE_GROUND] = _("Ground"),
    [TYPE_ROCK] = _("Rock"),     [TYPE_BUG] = _("Bug"),        [TYPE_GHOST] = _("Ghost"),   [TYPE_STEEL] = _("Steel"),     [TYPE_MYSTERY] = _("???"),
    [TYPE_FIRE] = _("Fire"),     [TYPE_WATER] = _("Water"),    [TYPE_GRASS] = _("Grass"),   [TYPE_ELECTRIC] = _("Electr"), [TYPE_PSYCHIC] = _("Psychc"),
    [TYPE_ICE] = _("Ice"),       [TYPE_DRAGON] = _("Dragon"),  [TYPE_DARK] = _("Dark"),     [TYPE_FAIRY] = _("Fairy"),     [TYPE_STELLAR] = _("Stellr"),
};
...
const struct TrainerMoney gTrainerMoneyTable[] = {
    ...
    {TRAINER_CLASS_MONOTYPE_CHAMPION, 45},
    {0xFF, 5},
};
...
const struct TrainerBall gTrainerBallTable[] = {
    ...
    {TRAINER_CLASS_MONOTYPE_CHAMPION, ITEM_ULTRA_BALL},
    {0xFF, ITEM_POKE_BALL},
};
...
static void CB2_InitBattleInternal(void) {
...
#else
    if (!gIsDebugBattle) gBattleTerrain = BattleSetup_GetTerrainId();
#endif
    if (gBattleTypeFlags & BATTLE_TYPE_RECORDED) gBattleTerrain = BATTLE_TERRAIN_BUILDING;
    ...
    if (!gIsDebugBattle) {
        if (!(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED))) {
            CreateNPCTrainerParty(&gEnemyParty[0], gTrainerBattleOpponent_A, TRUE);
            if (gBattleTypeFlags & BATTLE_TYPE_TWO_OPPONENTS) CreateNPCTrainerParty(&gEnemyParty[PARTY_SIZE / 2], gTrainerBattleOpponent_B, FALSE);
            SetWildMonHeldItem();
        }
    }
#endif
    ...
    for (i = 0; i < PARTY_SIZE; i++) AdjustFriendship(&gPlayerParty[i], FRIENDSHIP_EVENT_LEAGUE_BATTLE);
...
#define BUFFER_PARTY_VS_SCREEN_STATUS(party, flags, i)                                   \
    for ((i) = 0; (i) < PARTY_SIZE; (i)++) {                                             \
        ...
        if (species == SPECIES_NONE) continue;                                           \
                                                                                         \
        /* Is healthy mon? */                                                            \
        if (species != SPECIES_EGG && hp != 0 && status == 0) (flags) |= 1 << (i) * 2;   \
                                                                                         \
        if (species == SPECIES_NONE) /* Redundant */                                     \
            continue;                                                                    \
                                                                                         \
        /* Is Egg or statused? */                                                        \
        if (hp != 0 && (species == SPECIES_EGG || status != 0)) (flags) |= 2 << (i) * 2; \
                                                                                         \
        if (species == SPECIES_NONE) /* Redundant */                                     \
            continue;                                                                    \
                                                                                         \
...
static void FindLinkBattleMaster(u8 numPlayers, u8 multiPlayerId) {
    ...
    if (found == 0) {
        ...
        for (i = 0; i < numPlayers; i++) {
            if (gBlockRecvBuffer[0][0] != gBlockRecvBuffer[i][0]) break;
        }
        ...
        if (found == 0) {
            ...
            for (i = 0; i < numPlayers; i++) {
                ...
                if (gBlockRecvBuffer[i][0] > 0x300 && i != multiPlayerId) break;
            }
...
static void CB2_HandleStartBattle(void) {
    ...
    switch (gBattleCommunication[MULTIUSE_STATE]) {
        case 0:
            ...
            if (gWirelessCommType) LoadWirelessStatusIndicatorSpriteGfx();
            break;
        case 1:
            if (gBattleTypeFlags & BATTLE_TYPE_LINK) {
                    ...
                    if (IsLinkTaskFinished()) {
                        ...
                        *(&gBattleStruct->multiBuffer.linkBattlerHeader.versionSignatureHi) = 3;
                        ...
                        SendBlock(bitmask_all_link_players_but_self(),
                                  &gBattleStruct->multiBuffer.linkBattlerHeader,
                                  sizeof(gBattleStruct->multiBuffer.linkBattlerHeader));
                        gBattleCommunication[MULTIUSE_STATE] = 2;
                    }
                    if (gWirelessCommType) CreateWirelessStatusIndicatorSprite(0, 0);
                }
            } else {
                if (!(gBattleTypeFlags & BATTLE_TYPE_RECORDED)) gBattleTypeFlags |= BATTLE_TYPE_IS_MASTER;
        ...
        case 10:
        case 14:
            if (--gBattleCommunication[1] == 0) gBattleCommunication[MULTIUSE_STATE]++;
            break;
    }
}
...
static void CB2_HandleStartMultiPartnerBattle(void) {
    ...
    switch (gBattleCommunication[MULTIUSE_STATE]) {
        ...
        case 12:
            if ((GetBlockReceivedStatus() & 3) == 3) {
                ResetBlockReceivedFlags();
                if (GetMultiplayerId() != 0) memcpy(gEnemyParty + 4, gBlockRecvBuffer[0], sizeof(struct Pokemon) * 2);
...
static void sub_80379F8(u8 arrayIdPlus) {
    ...
    for (i = 0; i < (int)ARRAY_COUNT(gMultiPartnerParty); i++) {
        ...
        if (GetMonData(&gPlayerParty[arrayIdPlus + i], MON_DATA_LANGUAGE) != LANGUAGE_JAPANESE) PadNameString(gMultiPartnerParty[i].nickname, CHAR_SPACE);
    }
    ...
    switch (gBattleCommunication[MULTIUSE_STATE]) {
        case 0:
            if (gReceivedRemoteLinkPlayers != 0 && IsLinkTaskFinished()) {
                ...
                sub_80379F8(0);
                SendBlock(
                    bitmask_all_link_players_but_self(), sMultiPartnerPartyBuffer, sizeof(struct UnknownPokemonStruct4) * ARRAY_COUNT(gMultiPartnerParty));
                gBattleCommunication[MULTIUSE_STATE]++;
            }
            break;
        case 1:
            if ((GetBlockReceivedStatus() & r4) == r4) {
                ResetBlockReceivedFlags();
                for (i = 0; i < numPlayers; i++) {
                    if (i == playerMultiplierId) continue;
...
static void sub_8038538(struct Sprite* sprite) {
    ...
    switch (sprite->data[0]) {
        ...
        case 2:
            sprite->data[1]--;
            if (sprite->data[1] == 20) SetMainCallback2(CB2_InitBattle);
            break;
    }
}
...
static u8 CreateNPCTrainerParty(struct Pokemon* party, u16 trainerNum, bool8 firstTrainer) {
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_TRAINER && difficultySetting == DIFFICULTY_HELL) RegisterTrainerBattleEvents(trainerNum);
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_TRAINER && !(gBattleTypeFlags & (BATTLE_TYPE_FRONTIER | BATTLE_TYPE_EREADER_TRAINER | BATTLE_TYPE_TRAINER_HILL))) {
        ...
        for (i = 0; i < monsCount; i++) {
            ...
            if (trainerNum == TRAINER_OLDPLAYER) {
                species = 1 + Random() % 499;  // avoid SPECIES_NONE
...
void VBlankCB_Battle(void) {
    ...
    if (!(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_FRONTIER | BATTLE_TYPE_RECORDED))) Random();
...
void CB2_InitEndLinkBattle(void) {
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_FRONTIER) {
        ...
        InitBattleBgsVideo();
        LoadCompressedPalette(gTheme_Dark_BattleTextboxPalette, 0, 64);
...
static void EndLinkBattleInSteps(void) {
    ...
    switch (gBattleCommunication[MULTIUSE_STATE]) {
        ...
        case 3:
            ...
            for (i = 0; i < 2; i++) LoadChosenBattleElement(i);

            BeginNormalPaletteFade(PALETTES_ALL, 0, 0x10, 0, RGB_BLACK);
            gBattleCommunication[MULTIUSE_STATE]++;
            break;
        case 4:
            if (!gPaletteFade.active) gBattleCommunication[MULTIUSE_STATE]++;
            break;
        case 5:
            if (!FuncIsActiveTask(Task_ReconnectWithLinkPlayers)) gBattleCommunication[MULTIUSE_STATE]++;
            break;
        case 6:
        ...
        case 7:
            if (!IsTextPrinterActive(0)) {
                if (IsLinkTaskFinished() == TRUE) gBattleCommunication[MULTIUSE_STATE]++;
            }
            break;
        case 8:
            if (!gWirelessCommType) SetCloseLinkCallback();
            gBattleCommunication[MULTIUSE_STATE]++;
            break;
        case 9:
...
static void AskRecordBattle(void) {
    switch (gBattleCommunication[MULTIUSE_STATE]) {
        case STATE_INIT:
            ...
            break;
        case STATE_LINK:
            if (gMain.anyLinkBattlerHasFrontierPass && gReceivedRemoteLinkPlayers == 0) CreateTask(Task_ReconnectWithLinkPlayers, 5);
            gBattleCommunication[MULTIUSE_STATE]++;
            break;
        ...
        case STATE_WAIT_END:
            if (--gBattleCommunication[1] == 0) {
                if (gMain.anyLinkBattlerHasFrontierPass && !gWirelessCommType) SetCloseLinkCallback();
                gBattleCommunication[MULTIUSE_STATE]++;
            }
            break;
        case STATE_END:
...
static void TryCorrectShedinjaLanguage(struct Pokemon* mon) {
    ...
    if (GetMonData(mon, MON_DATA_SPECIES) == SPECIES_SHEDINJA && GetMonData(mon, MON_DATA_LANGUAGE) != language) {
        ...
        if (StringCompareWithoutExtCtrlCodes(nickname, sText_ShedinjaJpnName) == 0) SetMonData(mon, MON_DATA_LANGUAGE, &language);
    }
}

u32 GetBattleWindowTemplatePixelWidth(u32 setId, u32 tableId) { return gBattleWindowTemplates[setId][tableId].width * 8; }
...
static void SpriteCB_AnimFaintOpponent(struct Sprite* sprite) {
    ...
    if (--sprite->data[4] == 0) {
        ...
        {
            u8* dst =
                gMonSpritesGfxPtr->sprites.byte[GetBattlerPosition(sprite->sBattler)] + (gBattleMonForms[sprite->sBattler] << 11) + (sprite->data[3] << 8);

            for (i = 0; i < 0x100; i++) *(dst++) = 0;
...
void SpriteCb_OpponentMonFromBall(struct Sprite* sprite) {
    if (sprite->affineAnimEnded) {
        if (!(gHitMarker & HITMARKER_NO_ANIMATIONS) || gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED_LINK)) {
            if (HasTwoFramesAnimation(sprite->sSpeciesId)) StartSpriteAnim(sprite, 1);
        }
...
void SpriteCB_BattleSpriteStartSlideLeft(struct Sprite* sprite) { sprite->callback = SpriteCB_BattleSpriteSlideLeft; }
...
// Unused
static void sub_80105DC(struct Sprite* sprite) { sprite->callback = SpriteCallbackDummy_3; }
...
void DoBounceEffect(u8 battler, u8 which, s8 delta, s8 amplitude) {
    ...
    switch (which) {
        ...
        default:
            if (gBattleSpritesDataPtr->healthBoxesData[battler].healthboxIsBouncing) return;
            break;
        case BOUNCE_MON:
            if (gBattleSpritesDataPtr->healthBoxesData[battler].battlerIsBouncing) return;
            break;
    }
...
static void SpriteCB_BounceEffect(struct Sprite* sprite) {
    ...
    if (sprite->sWhich == BOUNCE_HEALTHBOX && bouncerSpriteId != 0xFF) gSprites[bouncerSpriteId].y2 = y;
}
...
void SpriteCb_PlayerMonFromBall(struct Sprite* sprite) {
    if (sprite->affineAnimEnded) BattleAnimateBackSprite(sprite, sprite->sSpeciesId);
}

void sub_8039E60(struct Sprite* sprite) {
    sub_8039E9C(sprite);
    if (sprite->animEnded) sprite->callback = SpriteCallbackDummy_3;
}
...
void sub_8039E9C(struct Sprite* sprite) {
    if (sprite->animDelayCounter == 0) sprite->centerToCornerVecX = gUnknown_0831ACE0[sprite->animCmdIndex];
}
...
static void BattleMainCB1(void) {
    ...
    for (gActiveBattler = 0; gActiveBattler < gBattlersCount; gActiveBattler++) gBattlerControllerFuncs[gActiveBattler]();
}
...
static void BattleStartClearSetData(void) {
    ...
    if (!(gBattleTypeFlags & BATTLE_TYPE_RECORDED)) {
        if (!(gBattleTypeFlags & BATTLE_TYPE_LINK) && gSaveBlock2Ptr->optionsBattleSceneOff == TRUE) gHitMarker |= HITMARKER_NO_ANIMATIONS;
...
void SwitchInClearSetData(void) {
    ...
    if (gBattleMoves[gCurrentMove].effect != EFFECT_BATON_PASS) {
        ...
        for (i = 0; i < gBattlersCount; i++) {
            if ((gBattleMons[i].status2 & STATUS2_ESCAPE_PREVENTION) && gVolatileStructs[i].battlerPreventingEscape == gActiveBattler)
                gBattleMons[i].status2 &= ~STATUS2_ESCAPE_PREVENTION;
            if ((gStatuses3[i] & STATUS3_ALWAYS_HITS) && gVolatileStructs[i].battlerWithSureHit == gActiveBattler) {
                gStatuses3[i] &= ~STATUS3_ALWAYS_HITS;
                gVolatileStructs[i].battlerWithSureHit = 0;
            }
        }
    }
    if (gBattleMoves[gCurrentMove].effect == EFFECT_BATON_PASS) {
        gBattleMons[gActiveBattler].status2 &=
            (STATUS2_CONFUSION | STATUS2_SUBSTITUTE | STATUS2_ESCAPE_PREVENTION | STATUS2_CURSED | STATUS2_POWDER | STATUS2_ENRAGED);
        gStatuses3[gActiveBattler] &=
            (STATUS3_LEECHSEED_BATTLER | STATUS3_LEECHSEED | STATUS3_ALWAYS_HITS | STATUS3_PERISH_SONG | STATUS3_ROOTED | STATUS3_GASTRO_ACID |
             STATUS3_EMBARGO | STATUS3_TELEKINESIS | STATUS3_MAGNET_RISE | STATUS3_HEAL_BLOCK | STATUS3_AQUA_RING | STATUS3_POWER_TRICK);
        ...
        for (i = 0; i < gBattlersCount; i++) {
            if (GetBattlerSide(gActiveBattler) != GetBattlerSide(i) && (gStatuses3[i] & STATUS3_ALWAYS_HITS) != 0 &&
                (gVolatileStructs[i].battlerWithSureHit == gActiveBattler)) {
                gStatuses3[i] &= ~(STATUS3_ALWAYS_HITS);
                gStatuses3[i] |= STATUS3_ALWAYS_HITS_TURN(2);
            }
        }
        if (gStatuses3[gActiveBattler] & STATUS3_POWER_TRICK) SWAP(gBattleMons[gActiveBattler].attack, gBattleMons[gActiveBattler].defense, i);
    ...
    // Remove any queued out-of-turn attacks
    {
        ...
        // Remove any queued switches (must not be nested in the loop above:
        // sharing its index and not resetting the count corrupts the queue)
        previousCount = gQueuedSwitchCount;
        gQueuedSwitchCount = 0;
        for (i = 1; i <= previousCount; i++) {
            if (gQueuedSwitchData[i].switchingBattler == gActiveBattler) continue;
            if (gQueuedSwitchData[i].sourceBattler == gActiveBattler) continue;
    ...
    if (gBattleMoves[gCurrentMove].effect == EFFECT_BATON_PASS) {
        gVolatileStructs[gActiveBattler].started = VolatileStructCopy.started;
        gVolatileStructs[gActiveBattler].substituteHP = VolatileStructCopy.substituteHP;
        gVolatileStructs[gActiveBattler].battlerWithSureHit = VolatileStructCopy.battlerWithSureHit;
        ...
        gVolatileStructs[gActiveBattler].drenched = VolatileStructCopy.drenched;
    } else if (gBattleMoves[gCurrentMove].effect == EFFECT_SHED_TAIL) {
        gVolatileStructs[gActiveBattler].substituteHP = VolatileStructCopy.substituteHP;
    }
    ...
    gLastChosenMove[gActiveBattler] = 0;
    ...
    if (gActiveBattler == gBattleStruct->stickyWebUser) gBattleStruct->stickyWebUser = 0xFF;  // Switched into sticky web user slot so reset it

    for (i = 0; i < gBattlersCount; i++) {
        if (i != gActiveBattler && GetBattlerSide(i) != GetBattlerSide(gActiveBattler)) gBattleStruct->lastTakenMove[i] = 0;
    ...
    gTurnStructs[gActiveBattler].physicalDmg = 0;
...
void FaintClearSetData(void) {
    ...
    if (gActiveBattler == gBattleStruct->stickyWebUser) gBattleStruct->stickyWebUser = 0xFF;  // User of sticky web fainted, so reset the stored battler ID
    ...
    if (GetBattlerSide(gActiveBattler) == B_SIDE_PLAYER) UndoMegaEvolution(gBattlerPartyIndexes[gActiveBattler]);
...
static void DoBattleIntro(void) {
    ...
    switch (*state) {
        ...
        case 3:  // Wait for intro slide.
            if (!gBattleControllerExecFlags) (*state)++;
            break;
        case 4:  // Copy battler data gotten in cases 0 and 1. Draw trainer/mon sprite.
            for (gActiveBattler = 0; gActiveBattler < gBattlersCount; gActiveBattler++) {
                if ((gBattleTypeFlags & BATTLE_TYPE_SAFARI) && GetBattlerSide(gActiveBattler) == B_SIDE_PLAYER) {
                    ...
                    gBattleMons[gActiveBattler].status2 = 0;
                    for (i = 0; i < NUM_BATTLE_STATS; i++) gBattleMons[gActiveBattler].statStages[i] = 6;
                }
        ...
        case 5:  // draw party summary in trainer battles
            if (!gBattleControllerExecFlags) {
                ...
                for (i = 0; i < PARTY_SIZE; i++) {
                    if (GetMonData(&gEnemyParty[i], MON_DATA_SPECIES2) == SPECIES_NONE || GetMonData(&gEnemyParty[i], MON_DATA_SPECIES2) == SPECIES_EGG) {
        ...
        case 15:  // wait for wild battle message
            if (!IsBattlerMarkedForControllerExec(GetBattlerAtPosition(B_POSITION_PLAYER_LEFT))) (*state)++;
            break;
        case 16:  // print player sends out
            if (!(gBattleTypeFlags & BATTLE_TYPE_SAFARI)) {
                ...
                if (B_FAST_INTRO && gBattleTypeFlags & BATTLE_TYPE_TRAINER &&
        ...
        case 17:  // wait for player send out message
            if (!(gBattleTypeFlags & BATTLE_TYPE_LINK && gBattleControllerExecFlags)) {
                ...
                if (!IsBattlerMarkedForControllerExec(gActiveBattler)) (*state)++;
            }
            break;
            ...
            if (!gBattleControllerExecFlags) {
                for (gActiveBattler = 0; gActiveBattler < gBattlersCount; gActiveBattler++) {
                    if (GetBattlerSide(gActiveBattler) == B_SIDE_OPPONENT &&
                        !(gBattleTypeFlags &
                          (BATTLE_TYPE_EREADER_TRAINER | BATTLE_TYPE_FRONTIER | BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED_LINK | BATTLE_TYPE_TRAINER_HILL))) {
...
static void TryDoEventsBeforeFirstTurn(void) {
    ...
    if (!(gBattleTypeFlags & BATTLE_TYPE_SAFARI)) {
        for (i = 0; i < gBattlersCount; i++) {
            if (gBattleMons[i].hp == 0 || gBattleMons[i].species == SPECIES_NONE) gAbsentBattlerFlags |= gBitTable[i];
        }
    }

    if (!gBattleStruct->overworldWeatherDone && AbilityBattleEffects(ABILITYEFFECT_SWITCH_IN_WEATHER, 0, 0, TRUE, 0) != 0) {
    ...
    while (gBattleStruct->switchInItemsCounter < gBattlersCount) {
        ...
        if (ItemBattleEffects(ITEMEFFECT_ON_SWITCH_IN, gBattlerAttacker, FALSE)) return;
    }
    ...
    for (i = 0; i < MAX_BATTLERS_COUNT; i++) {
        *(gBattleStruct->monToSwitchIntoId + i) = PARTY_SIZE;
        ...
        if (!IsBattlerAIControlled(i)) gBattleStruct->appearedInBattle |= gBitTable[gBattlerPartyIndexes[i]];
    }
    TurnValuesCleanUp(FALSE);
    for (i = 0; i < gBattlersCount; i++) {
    ...
    *(&gBattleStruct->field_91) = gAbsentBattlerFlags;
    ...
    for (i = 0; i < gBattlersCount; i++) gBattleMons[i].status2 &= ~(STATUS2_FLINCHED);
...
static void HandleEndTurn_ContinueBattle(void) {
    ...
    if (gBattleControllerExecFlags == 0) {
        ...
        for (i = 0; i < gBattlersCount; i++) {
            ...
            if ((gBattleMons[i].status1 & STATUS1_SLEEP) && (gBattleMons[i].status2 & STATUS2_MULTIPLETURNS)) CancelMultiTurnMoves(i);
        }
        gBattleStruct->turnEffectsTracker = 0;
        gBattleStruct->turnEffectsBattlerId = 0;
        gBattleStruct->wishPerishSongState = 0;
...
void BattleTurnPassed(void) {
    ...
    for (i = 0; i < 5; i++) gBattleCommunication[i] = 0;
    ...
    for (i = 0; i < MAX_BATTLERS_COUNT; i++) gBattleStruct->monToSwitchIntoId[i] = PARTY_SIZE;
...
u8 IsRunningFromBattleImpossible(void) {
    ...
    if (GetBattlerPosition(gActiveBattler) == B_POSITION_PLAYER_RIGHT && WILD_DOUBLE_BATTLE &&
        IsBattlerAlive(
            GetBattlerAtPosition(B_POSITION_PLAYER_LEFT)))  // The second pokemon cannot run from a double wild battle, unless it's the only alive mon.
    {
        gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_CANT_ESCAPE;
        return 1;
    }

    if (holdEffect == HOLD_EFFECT_SHED_SHELL) return 0;
#if B_GHOSTS_ESCAPE >= GEN_6
    if (IS_BATTLER_OF_TYPE(gActiveBattler, TYPE_GHOST)) return 0;
#endif
    if (gBattleTypeFlags & BATTLE_TYPE_LINK) return 0;
    if (BattlerHasAbility(gActiveBattler, ABILITY_RUN_AWAY, FALSE)) return 0;
...
void SwitchPartyOrder(u8 battler) {
    ...
    // gBattleStruct->battlerPartyOrders[battler][i]
...
static void HandleTurnActionSelectionState(void) {
    ...
    for (gActiveBattler = 0; gActiveBattler < gBattlersCount; gActiveBattler++) {
        ...
        switch (gBattleCommunication[gActiveBattler]) {
                ...
                if (gBattleTypeFlags & BATTLE_TYPE_MULTI || (position & BIT_FLANK) == B_FLANK_LEFT ||
                    ...
                    if (gBattleStruct->field_91 & gBitTable[gActiveBattler]) {
                        ...
                        if (gBattleMons[gActiveBattler].status2 & STATUS2_MULTIPLETURNS || gBattleMons[gActiveBattler].status2 & STATUS2_RECHARGE) {
                            ...
                            gBattleCommunication[gActiveBattler] = STATE_WAIT_ACTION_CONFIRMED_STANDBY;
                        } else if (WILD_DOUBLE_BATTLE && position == B_POSITION_PLAYER_RIGHT &&
                            ...
                            BtlController_EmitChooseAction(
                                0, gChosenActionByBattler[0], gBattleResources->bufferB[0][1] | (gBattleResources->bufferB[0][2] << 8));
                            MarkBattlerForControllerExec(gActiveBattler);
            ...
            case STATE_WAIT_ACTION_CHOSEN:  // Try to perform an action.
                if (!(gBattleControllerExecFlags & ((gBitTable[gActiveBattler]) | (0xF << 28) | (gBitTable[gActiveBattler] << 4) |
                    ...
                    switch (gBattleResources->bufferB[gActiveBattler][1]) {
                        ...
                        case B_ACTION_USE_ITEM:
                            ...
                            if (FlagGet(FLAG_SYS_NO_BAG_USE) || gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_FRONTIER_NO_PYRAMID |
                        ...
                        case B_ACTION_SWITCH:
                            *(gBattleStruct->battlerPartyIndexes + gActiveBattler) = gBattlerPartyIndexes[gActiveBattler];
                            if (gBattleTypeFlags & BATTLE_TYPE_ARENA || !CanBattlerEscape(gActiveBattler)) {
                                BtlController_EmitChoosePokemon(
                                    0, PARTY_ACTION_CANT_SWITCH, PARTY_SIZE, ABILITY_NONE, gBattleStruct->battlerPartyOrders[gActiveBattler]);
                            } else if ((i = IsAbilityPreventingEscape(gActiveBattler))) {
                                BtlController_EmitChoosePokemon(0,
                                                                ((i - 1) << 4) | PARTY_ACTION_ABILITY_PREVENTS,
                                                                PARTY_SIZE,
                                                                gBattleMons[i - 1].abilities[0],
                                ...
                                if (gActiveBattler == 2 && gChosenActionByBattler[0] == B_ACTION_SWITCH)
                                    ...
                                    BtlController_EmitChoosePokemon(
                                        0, PARTY_ACTION_CHOOSE_MON, PARTY_SIZE, ABILITY_NONE, gBattleStruct->battlerPartyOrders[gActiveBattler]);
                            }
                            MarkBattlerForControllerExec(gActiveBattler);
                            break;
                        ...
                        case B_ACTION_CANCEL_PARTNER:
                            ...
                            if (gBattleMons[GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(gActiveBattler)))].status2 & STATUS2_MULTIPLETURNS ||
                                ...
                                RecordedBattle_ClearBattlerAction(GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(gActiveBattler))), 1);
                                       ...
                                       (gRoundStructs[GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(gActiveBattler)))].noValidMoves ||
                                        gVolatileStructs[GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(gActiveBattler)))].encoredMove)) {
                                RecordedBattle_ClearBattlerAction(GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(gActiveBattler))), 1);
...
#ifdef DEBUG_BUILD
                    else if (FlagGet(FLAG_SYS_AUTOWIN) && gBattleTypeFlags & BATTLE_TYPE_TRAINER &&
                             !(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED_LINK)) &&
                             gBattleResources->bufferB[gActiveBattler][1] == B_ACTION_RUN) {
                        gBattleCommunication[gActiveBattler]++;
                    }
#endif
                    else if (gBattleTypeFlags & BATTLE_TYPE_TRAINER && !(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED_LINK)) &&
                             gBattleResources->bufferB[gActiveBattler][1] == B_ACTION_RUN) {
                        gSelectionBattleScripts[gActiveBattler] = BattleScript_AskIfWantsToForfeitMatch;
                        gBattleCommunication[gActiveBattler] = STATE_SELECTION_SCRIPT_MAY_RUN;
                        *(gBattleStruct->selectionScriptFinished + gActiveBattler) = FALSE;
                    ...
                    switch (gChosenActionByBattler[gActiveBattler]) {
                                ...
                                default:
                                    ...
                                    if ((gBattleResources->bufferB[gActiveBattler][2] | (gBattleResources->bufferB[gActiveBattler][3] << 8)) == 0xFFFF) {
                                        ...
                                        gBattleStruct->chosenMovePositions[gActiveBattler] =
                                            gBattleResources->bufferB[gActiveBattler][2] & ~(RET_MEGA_EVOLUTION);
                                        gChosenMoveByBattler[gActiveBattler] =
                                            gBattleMons[gActiveBattler].moves[gBattleStruct->chosenMovePositions[gActiveBattler]];
                                        gBattleStruct->moveTarget[gActiveBattler] = gBattleResources->bufferB[gActiveBattler][3];
                        ...
                        case B_ACTION_USE_ITEM:
                            if ((gBattleResources->bufferB[gActiveBattler][1] | (gBattleResources->bufferB[gActiveBattler][2] << 8)) == 0) {
                                ...
                                if (ItemId_GetPocket(gLastUsedItem) == POCKET_POKE_BALLS) gBattleStruct->throwingPokeBall = TRUE;
                                gBattleCommunication[gActiveBattler]++;
                            }
                            break;
                        case B_ACTION_SWITCH:
    ...
    if (gBattleCommunication[ACTIONS_CONFIRMED_COUNT] == gBattlersCount) {
        ...
        if (gBattleTypeFlags & BATTLE_TYPE_INGAME_PARTNER) {
            for (i = 0; i < gBattlersCount; i++) {
                if (gChosenActionByBattler[i] == B_ACTION_SWITCH) SwitchPartyOrderInGameMulti(i, *(gBattleStruct->monToSwitchIntoId + i));
            }
        }
    }
}
...
static bool8 AllAtActionConfirmed(void) {
    ...
    for (count = 0, i = 0; i < gBattlersCount; i++) {
        if (gBattleCommunication[i] == STATE_WAIT_ACTION_CONFIRMED) count++;
    }
...
u32 GetBattlerTotalSpeedStat(u8 battlerId, u8 calcType, MoveEnum move) {
    ...
    if (holdEffect == HOLD_EFFECT_MACHO_BRACE || holdEffect == HOLD_EFFECT_POWER_ITEM)
        ...
        speed = (speed * 150) / 100;
    else if (holdEffect == HOLD_EFFECT_QUICK_POWDER && gBattleMons[battlerId].species == SPECIES_DITTO &&
...
s8 GetMovePriority(u32 battlerId, MoveEnum move, u32 target) {
    ...
    if (move == MOVE_RAZOR_WIND && (gSideStatuses[0] & SIDE_STATUS_TAILWIND || gSideStatuses[1] & SIDE_STATUS_TAILWIND)) priority++;
...
union SpeedValue GetMoveSpeed(int battler, int ignoreChosenMove) {
    ...
    if (!ignoreChosenMove) {
        ...
        priority += GetChosenMovePriority(battler, GetFullChosenTarget(battler, move));
        if (priority > 15)
            priority = 15;
        else if (priority < 0)
            priority = 0;
    ...
    if (!ignoreChosenMove && !quash && MYCELIUM_MIGHT_AFFECTED(battler, GetChosenMove(battler))) speedValue.goesLastNegation++;
...
u8 GetWhoStrikesFirst(u8 battler1, u8 battler2, bool8 ignoreChosenMoves) {
    ...
    battler1Speed = GetMoveSpeed(battler1, ignoreChosenMoves).comparable;
    battler2Speed = GetMoveSpeed(battler2, ignoreChosenMoves).comparable;
...
static void TurnValuesCleanUp(bool8 turnPassOnly) {
    ...
    for (i = 0; i < gBattlersCount; i++) {
        if (!turnPassOnly) {
            ...
            if (gVolatileStructs[i].isFirstTurn) gVolatileStructs[i].isFirstTurn--;

            if (gVolatileStructs[i].rechargeTimer) {
                gVolatileStructs[i].rechargeTimer--;
                if (gVolatileStructs[i].rechargeTimer == 0) gBattleMons[i].status2 &= ~(STATUS2_RECHARGE);
            }
        }
...
void TurnStructsClear(void) { ZERO(gTurnStructs) }
...
static void CheckMegaEvolutionBeforeTurn(void) {
    if (!(gHitMarker & HITMARKER_RUN)) {
        while (gBattleStruct->mega.battlerId < gBattlersCount) {
            ...
            if (gBattleStruct->mega.toEvolve & gBitTable[gActiveBattler] && !(gRoundStructs[gActiveBattler].noValidMoves)) {
                gBattleStruct->mega.toEvolve &= ~(gBitTable[gActiveBattler]);
...
static void CheckFocusPunch_ClearVarsBeforeTurnStarts(void) {
    ...
    if (!(gHitMarker & HITMARKER_RUN)) {
        while (gBattleStruct->focusPunchBattlerId < gBattlersCount) {
            ...
            if (!(gBattleMons[gActiveBattler].status1 & STATUS1_SLEEP) && !(gRoundStructs[gActiveBattler].noValidMoves)) {
...
static void RunTurnActionsFunctions(void) {
    if (gBattleOutcome != 0) gCurrentActionFuncId = B_ACTION_FINISHED;
...
static void HandleEndTurn_RanFromBattle(void) {
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_FRONTIER && gBattleTypeFlags & BATTLE_TYPE_TRAINER) {
        ...
        switch (gRoundStructs[gBattlerAttacker].fleeFlag) {
            default:
...
static void HandleEndTurn_FinishBattle(void) {
    ...
    if (gCurrentActionFuncId == B_ACTION_TRY_FINISH || gCurrentActionFuncId == B_ACTION_FINISHED) {
        if (!(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED_LINK | BATTLE_TYPE_FIRST_BATTLE | BATTLE_TYPE_SAFARI | BATTLE_TYPE_EREADER_TRAINER |
        ...
        if (!(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED_LINK | BATTLE_TYPE_TRAINER | BATTLE_TYPE_FIRST_BATTLE | BATTLE_TYPE_SAFARI |
                                  BATTLE_TYPE_FRONTIER | BATTLE_TYPE_EREADER_TRAINER | BATTLE_TYPE_WALLY_TUTORIAL)) &&
        ...
        if (gBattleTypeFlags & BATTLE_TYPE_TRAINER || B_ALWAYS_RESTORE_ITEMS) TryRestoreStolenItems();
#endif
        for (i = 0; i < PARTY_SIZE; i++) {
            UndoMegaEvolution(i);
...
static void ReturnFromBattleToOverworld(void) {
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_LINK && gReceivedRemoteLinkPlayers != 0) return;
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_ROAMER) {
        ...
        if ((gBattleOutcome == B_OUTCOME_WON) ||
            gBattleOutcome == B_OUTCOME_CAUGHT)  // Bug: When Roar is used by roamer, gBattleOutcome is B_OUTCOME_PLAYER_TELEPORTED (5).
...
void ApplyTypeOverrideInformation(MoveEnum move, int battlerAtk, int moveType, int ateBoost) {
    ...
    if (GetBattlerHoldEffect(battlerAtk, TRUE) == HOLD_EFFECT_GEMS && moveType == ItemId_GetSecondaryId(gBattleMons[battlerAtk].item) &&
        ...
        gTurnStructs[battlerAtk].gemBoost = FALSE;
    }
}
...
// 500 * (num / den)^3, clamped to 1000. The original 32-bit formula overflowed for base stats above ~203
// and divided by zero for a stat of 0.
static u32 GetCubedStatChance(u32 caster, u32 num, u32 den) {
    u64 n = (u64)caster * 500 * num * num * num;
    u64 d = (u64)caster * den * den * den;
    if (d == 0) return 1000;
    n /= d;
    return n > 1000 ? 1000 : (u32)n;
}
...
u16 selectMoves(SpeciesEnum species, u8 i, u16 atk, u16 spAtk) {
    ...
    if (i == MAX_MON_MOVES - 1) {
        ...
        if (atk > spAtk) {
            chanceValue = GetCubedStatChance(caster, spAtk, atk);
            if ((atk * 3) > (spAtk * 4))
                split = 0;
            ...
            chanceValue = GetCubedStatChance(caster, atk, spAtk);
            if ((spAtk * 3) > (atk * 4))
                split = 1;
            else if (randomValue < chanceValue)
                split = 0;
...
bool32 IsWildMonSmart(void) { return (B_SMART_WILD_AI_FLAG != 0 && FlagGet(B_SMART_WILD_AI_FLAG)); }
