...
static const struct ListMenuItem sLevelMenuItems[] = {
    {gText_ExpandedPlaceholder_Empty, 0}, {gText_ExpandedPlaceholder_Empty, 1}, {gText_Exit, DAYCARE_LEVEL_MENU_EXIT}};
...
static const u8 *const sCompatibilityMessages[] = {gDaycareText_GetAlongVeryWell, gDaycareText_GetAlong, gDaycareText_DontLikeOther, gDaycareText_PlayOther};
...
u8 CountPokemonInDaycare(struct DayCare *daycare) {
    ...
    for (i = 0; i < DAYCARE_MON_COUNT; i++) {
        if (GetBoxMonData(&daycare->mons[i].mon, MON_DATA_SPECIES) != 0) count++;
    }
...
static s8 Daycare_FindEmptySpot(struct DayCare *daycare) {
    ...
    for (i = 0; i < DAYCARE_MON_COUNT; i++) {
        if (GetBoxMonData(&daycare->mons[i].mon, MON_DATA_SPECIES) == SPECIES_NONE) return i;
    }
...
static void ShiftDaycareSlots(struct DayCare *daycare) {
    ...
    if (GetBoxMonData(&daycare->mons[1].mon, MON_DATA_SPECIES) != SPECIES_NONE && GetBoxMonData(&daycare->mons[0].mon, MON_DATA_SPECIES) == SPECIES_NONE) {
    ...
    for (i = 0; i < MAX_LEVEL; i++) {
        ...
        if (TryIncrementMonLevel(mon)) {
            ...
            while ((learnedMove = MonTryLearningNewMove(mon, firstMove)) != 0) {
                firstMove = FALSE;
                if (learnedMove == MON_HAS_MAX_MOVES) DeleteFirstMoveAndGiveMoveToMon(mon, gMoveToLearn);
            }
        } else {
            break;
        }
...
u16 TakePokemonFromDaycare(void) { return TakeSelectedPokemonMonFromDaycareShiftSlots(&gSaveBlock1Ptr->daycare, gSpecialVar_0x8004); }
...
static u16 GetDaycareCostForMon(struct DayCare *daycare, u8 slotId) { return GetDaycareCostForSelectedMon(&daycare->mons[slotId]); }

void GetDaycareCost(void) { gSpecialVar_0x8005 = GetDaycareCostForMon(&gSaveBlock1Ptr->daycare, gSpecialVar_0x8004); }
...
static void ClearAllDaycareData(struct DayCare *daycare) {
    ...
    for (i = 0; i < DAYCARE_MON_COUNT; i++) ClearDaycareMon(&daycare->mons[i]);
...
u16 GetEggSpecies(SpeciesEnum species) {
    ...
    for (i = 0; i < 5; i++) {
        ...
        for (j = 1; j < NUM_SPECIES; j++) {
            if (!gEvolutionTable[j]) continue;
            for (k = 0; gEvolutionTable[j][k].method; k++) {
...
static s32 GetParentToInheritNature(struct DayCare *daycare) {
    ...
    for (i = 0; i < DAYCARE_MON_COUNT; i++) {
        if (GetBoxMonData(&daycare->mons[i].mon, MON_DATA_HELD_ITEM) == ITEM_EVERSTONE) everstoneCount++, parent = i;
    }
    ...
    if (parent < 0 || GetBoxMonData(&daycare->mons[parent].mon, MON_DATA_HELD_ITEM) != ITEM_EVERSTONE) {
        return -1;
    }
...
static void TryInheritAbility(struct Pokemon *egg, struct DayCare *daycare) {
    ...
    // search for female
    for (i = 0; i < DAYCARE_MON_COUNT; i++) {
        if (GetBoxMonGender(&daycare->mons[i].mon) == MON_FEMALE) femaleCount++, parent = i;
    }
...
static void _TriggerPendingDaycareEgg(struct DayCare *daycare) {
    ...
    // inherit nature
        ...
        do {
            ...
            if (wantedNature == GetNatureFromPersonality(personality) && personality != 0) break;  // found a personality with the same nature
...
void TriggerPendingDaycareEgg(void) { _TriggerPendingDaycareEgg(&gSaveBlock1Ptr->daycare); }
...
static void InheritIVs(struct Pokemon *egg, struct DayCare *daycare) {
    ...
    for (i = 0; i < DAYCARE_MON_COUNT; i++) {
        if (GetBoxMonData(&daycare->mons[i].mon, MON_DATA_HELD_ITEM) == ITEM_DESTINY_KNOT) destinyKnotCount++;
    }
...
static u8 GetBoxMonEggMoves(struct BoxPokemon *boxMon, u16 *eggMoves) { return 0; }
...
static void BuildEggMoveset(struct Pokemon *egg, struct BoxPokemon *father, struct BoxPokemon *mother) {
    ...
    for (i = 0; i < EGG_MOVES_ARRAY_COUNT; i++) sHatchedEggEggMoves[i] = MOVE_NONE;
    for (i = 0; i < EGG_LVL_UP_MOVES_ARRAY_COUNT; i++) sHatchedEggLevelUpMoves[i] = MOVE_NONE;
    ...
    for (i = 0; i < MAX_MON_MOVES; i++) {
        if (sHatchedEggFatherMoves[i] == MOVE_NONE) break;
        ...
        if (sHatchedEggFatherMoves[i] != MOVE_NONE) {
            for (j = 0; j < numEggMoves; j++) {
                if (sHatchedEggFatherMoves[i] == sHatchedEggEggMoves[j]) {
                    if (GiveMoveToMon(egg, sHatchedEggFatherMoves[i]) == MON_HAS_MAX_MOVES) DeleteFirstMoveAndGiveMoveToMon(egg, sHatchedEggFatherMoves[i]);
                    break;
                }
            }
        } else {
        ...
        if (sHatchedEggMotherMoves[i] != MOVE_NONE) {
            for (j = 0; j < numEggMoves; j++) {
                if (sHatchedEggMotherMoves[i] == sHatchedEggEggMoves[j]) {
                    if (GiveMoveToMon(egg, sHatchedEggMotherMoves[i]) == MON_HAS_MAX_MOVES) DeleteFirstMoveAndGiveMoveToMon(egg, sHatchedEggMotherMoves[i]);
                    break;
                }
            }
        } else {
    ...
    for (i = 0; i < MAX_MON_MOVES; i++) {
        if (sHatchedEggFinalMoves[i] == MOVE_NONE) break;
        for (j = 0; j < numLevelUpMoves; j++) {
            if (sHatchedEggLevelUpMoves[j] != MOVE_NONE && sHatchedEggFinalMoves[i] == sHatchedEggLevelUpMoves[j]) {
                if (GiveMoveToMon(egg, sHatchedEggFinalMoves[i]) == MON_HAS_MAX_MOVES) DeleteFirstMoveAndGiveMoveToMon(egg, sHatchedEggFinalMoves[i]);
                break;
            }
        }
    }
...
void RejectEggFromDayCare(void) { RemoveEggFromDayCare(&gSaveBlock1Ptr->daycare); }
...
static void GiveVoltTackleIfLightBall(struct Pokemon *mon, struct DayCare *daycare) {
    ...
    if (motherItem == ITEM_LIGHT_BALL || fatherItem == ITEM_LIGHT_BALL) {
        if (GiveMoveToMon(mon, MOVE_VOLT_TACKLE) == MON_HAS_MAX_MOVES) DeleteFirstMoveAndGiveMoveToMon(mon, MOVE_VOLT_TACKLE);
    }
}
...
static u16 DetermineEggSpeciesAndParentSlots(struct DayCare *daycare, u8 *parentSlots) {
    ...
    } else if (eggSpecies == SPECIES_GRENINJA_BATTLE_BOND || eggSpecies == SPECIES_GRENINJA_ASH) {
...
static void _GiveEggFromDaycare(struct DayCare *daycare) {
    ...
    if (species == SPECIES_PICHU) GiveVoltTackleIfLightBall(&egg, daycare);
...
void GiveEggFromDaycare(void) { _GiveEggFromDaycare(&gSaveBlock1Ptr->daycare); }

static bool8 TryProduceOrHatchEgg(struct DayCare *daycare) {
    u32 i, validEggs = 0;

    for (i = 0; i < DAYCARE_MON_COUNT; i++) {
        if (GetBoxMonData(&daycare->mons[i].mon, MON_DATA_SANITY_HAS_SPECIES)) daycare->mons[i].steps++, validEggs++;
    }

    // Check if an egg should be produced
    if (daycare->offspringPersonality == 0 && validEggs == DAYCARE_MON_COUNT && (daycare->mons[1].steps & 0xFF) == 0xFF) {
        u8 compatibility = ModifyBreedingScoreForOvalCharm(GetDaycareCompatibilityScore(daycare));
        if (compatibility > (Random() * 100u) / USHRT_MAX) TriggerPendingDaycareEgg();
    }
    ...
    if (++daycare->stepCounter == 255) {
        ...
        for (i = 0; i < gPlayerPartyCount; i++) {
            ...
            if (GetMonData(&gPlayerParty[i], MON_DATA_SANITY_IS_BAD_EGG)) continue;
...
bool8 ShouldEggHatch(void) { return TryProduceOrHatchEgg(&gSaveBlock1Ptr->daycare); }

static bool8 IsEggPending(struct DayCare *daycare) { return (daycare->offspringPersonality != 0); }
...
void GetDaycareMonNicknames(void) { _GetDaycareMonNicknames(&gSaveBlock1Ptr->daycare); }
...
static u8 GetDaycareCompatibilityScore(struct DayCare *daycare) {
    ...
    if (eggGroups[0][0] == EGG_GROUP_UNDISCOVERED || eggGroups[1][0] == EGG_GROUP_UNDISCOVERED) return PARENTS_INCOMPATIBLE;
    // two Ditto can't breed
    if (eggGroups[0][0] == EGG_GROUP_DITTO && eggGroups[1][0] == EGG_GROUP_DITTO) return PARENTS_INCOMPATIBLE;
...
static u8 GetDaycareCompatibilityScoreFromSave(void) { return GetDaycareCompatibilityScore(&gSaveBlock1Ptr->daycare); }
...
bool8 NameHasGenderSymbol(const u8 *name, u8 genderRatio) {
    ...
    for (i = 0; name[i] != EOS; i++) {
        if (name[i] == CHAR_MALE) symbolsCount[MALE]++;
        if (name[i] == CHAR_FEMALE) symbolsCount[FEMALE]++;
    }

    if (genderRatio == MON_MALE && symbolsCount[MALE] != 0 && symbolsCount[FEMALE] == 0) return TRUE;
    if (genderRatio == MON_FEMALE && symbolsCount[FEMALE] != 0 && symbolsCount[MALE] == 0) return TRUE;
...
static u8 *AppendGenderSymbol(u8 *name, u8 gender) {
    if (gender == MON_MALE) {
        if (!NameHasGenderSymbol(name, MON_MALE)) return StringAppend(name, gText_MaleSymbol4);
    } else if (gender == MON_FEMALE) {
        if (!NameHasGenderSymbol(name, MON_FEMALE)) return StringAppend(name, gText_FemaleSymbol4);
    }

    return StringAppend(name, gText_GenderlessSymbol);
}

static u8 *AppendMonGenderSymbol(u8 *name, struct BoxPokemon *boxMon) { return AppendGenderSymbol(name, GetBoxMonGender(boxMon)); }
