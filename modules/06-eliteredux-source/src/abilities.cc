...
constexpr Ability Impl<ABILITY_WONDER_GUARD> = {
    // Wonder Guard's type-immunity, unsuppressable, blocksAbilitySuppression and
    // randomizerBanned traits have all been moved to ABILITY_ABSOLUTE_GUARD.
    // This ability is intentionally left as a no-op.
};
...
constexpr Ability Impl<ABILITY_SPIDER_LAIR> = {
    .onEntry = +[](ON_ENTRY) -> int {
        CHECK_NOT(gSideStatuses[GetOppositeSide(battler)] & SIDE_STATUS_STICKY_WEB)
...
constexpr Ability Impl<ABILITY_SCRAPYARD> = {
    .onDefender = +[](ON_DEFENDER) -> int {
        CHECK(DidMoveHit())
        CHECK(IsMoveMakingContact(move, attacker))
        CHECK(gSideTimers[GetOppositeSide(battler)].spikesAmount < 3)
...
constexpr Ability Impl<ABILITY_TOXIC_DEBRIS> = {
    .onDefender = +[](ON_DEFENDER) -> int {
        CHECK(DidMoveHit())
        CHECK(IsMoveMakingContact(move, attacker))
        CHECK(gSideTimers[GetOppositeSide(battler)].toxicSpikesAmount < 2)
...
constexpr Ability Impl<ABILITY_LOOSE_ROCKS> = {
    .onDefender = +[](ON_DEFENDER) -> int {
        CHECK(DidMoveHit())
        CHECK(IsMoveMakingContact(move, attacker))
        CHECK_NOT(gSideStatuses[GetOppositeSide(battler)] & SIDE_STATUS_STEALTH_ROCK)
...
template <>
constexpr Ability Impl<ABILITY_SOVEREIGN_PROWESS> = {
    .onStat =
        +[](ON_STAT) {
            if (statId == STAT_ATK) *stat *= 2;
            else if (statId == STAT_SPATK) *stat *= 2;
        },
    .onChooseOffensiveStat = Impl<ABILITY_EQUINOX>.onChooseOffensiveStat,
};
...
template <>
constexpr Ability Impl<ABILITY_BLADEMASTER> = {
    .onOffensiveMultiplier =
        +[](ON_OFFENSIVE_MULTIPLIER) {
            if (DoesMoveMatchFlag(battler, move, moveType, MOVE_FLAG_KEEN_EDGE)) MUL(1.2);
        },
    .onCrit = +[](ON_CRIT) -> int {
        CHECK(DoesMoveMatchFlag(battler, move, GetTypeBeforeUsingMove(move, battler), MOVE_FLAG_KEEN_EDGE))
        return 1;
    },
};
...
constexpr Ability Impl<ABILITY_ARCANE_STANCE> = {
    ...
    .onMoveType = +[](ON_MOVE_TYPE) -> int {
        ...
        u16 bestMult = 0;
        for (int t = TYPE_NORMAL; t <= TYPE_FAIRY; t++) {
            if (t == TYPE_MYSTERY) continue;
            u16 mult = CalcTypeEffectivenessMultiplier(move, (Type)t, gBattlerAttacker, gBattlerTarget, FALSE);
            if (mult > bestMult || (mult == bestMult && (t == TYPE_STEEL || t == TYPE_FAIRY))) {
                bestMult = mult;
                best = (Type)t;
            }
        }
    ...
    .onBeforeAttack = +[](ON_BEFORE_ATTACK) -> int {
        if (gBattleMons[battler].species == SPECIES_SABREVOIR && move == MOVE_MYSTIC_AEGIS) {
            // Guard is required: the popup pattern re-executes this command, and unlike the
            // forme-change path the species/move condition never changes -> infinite loop.
            CHECK(CheckAndSetOncePerTurnAbility(battler, ability))
            SetStatChanger(STAT_DEF, 1);
            // 4th arg of SET_STATCHANGER2 is goesDown, not goesUp.
            SET_STATCHANGER2(gBattleScripting.savedStatChanger, STAT_SPDEF, 1, FALSE);
            BattleScriptCall(BattleScript_MysticAegisBrace);
            return TRUE;
        }
        ...
        switch (gBattleMons[battler].species) {
            default:
                return FALSE;
            case SPECIES_SABREVOIR:  // Shield -> Sword
                if (move == MOVE_MYSTIQUE_ARMAMENT) newSpecies = SPECIES_SABREVOIR_SWORD;
                break;
            case SPECIES_SABREVOIR_SWORD:  // Sword -> Shield
                if (move == MOVE_MYSTIC_AEGIS) newSpecies = SPECIES_SABREVOIR;
                break;
        }
        CHECK(newSpecies)
...
template <>
constexpr Ability Impl<ABILITY_ABSOLUTE_GUARD> = {
    .onEntry = Impl<ABILITY_CLOUD_NINE>.onEntry,
    .onAfterTypeEffectiveness =
        +[](ON_AFTER_TYPE_EFFECTIVENESS) {
            // DESIGN (deliberate): "absolute" guard. Everything that is not super effective is nullified,
            // including status moves that run typecalc (Thunder Wave, etc.). Do not add a power check here.
            if (*mod < GetSuperEffectiveMult()) *mod = 0;
        },
    .onAfterTypeEffectivenessFor = APPLY_ON_TARGET,
    .breakable = FALSE,
    .unsuppressable = TRUE,
    .blocksAbilitySuppression = TRUE,
    .randomizerBanned = TRUE,
    .magicGuard = TRUE,
};
...
constexpr Ability Impl<ABILITY_HOT_COALS> = {
    .onEntry = +[](ON_ENTRY) -> int {
        CHECK_NOT(gSideTimers[GetOppositeSide(battler)].hotCoals)

        gSideTimers[GetOppositeSide(battler)].hotCoals = TRUE;
...
template <>
constexpr Ability Impl<ABILITY_FLUFFIEST_ONE> = {
    // Elude + Fluffiest + Momentum + Impulse
    .onDefensiveMultiplier = Impl<ABILITY_FLUFFIEST>.onDefensiveMultiplier,
    .onChooseOffensiveStat =
        +[](ON_CHOOSE_OFFENSIVE_STAT) {
            Impl<ABILITY_MOMENTUM>.onChooseOffensiveStat(DELEGATE_CHOOSE_OFFENSIVE_STAT);
            Impl<ABILITY_IMPULSE>.onChooseOffensiveStat(DELEGATE_CHOOSE_OFFENSIVE_STAT);
        },
    .onChooseDefensiveStat = Impl<ABILITY_ELUDE>.onChooseDefensiveStat,
    .onChooseDefensiveStatFor = APPLY_ON_TARGET,
    .breakable = TRUE,
};
...
template <>
constexpr Ability Impl<ABILITY_PROPHETIC_DESTINY> = {
    // Dragon's Ritual + Long Reach + Queenly Majesty + Droideka
    .onImmune = Impl<ABILITY_QUEENLY_MAJESTY>.onImmune,
    .onBattlerFaints = Impl<ABILITY_DRAGONS_RITUAL>.onBattlerFaints,
    .onOffensiveMultiplier = Impl<ABILITY_LONG_REACH>.onOffensiveMultiplier,
    .onDefensiveMultiplier = Impl<ABILITY_DROIDEKA>.onDefensiveMultiplier,
    .onCrit = Impl<ABILITY_DROIDEKA>.onCrit,
    .onImmuneFor = APPLY_ON_ALLY,
    .onBattlerFaintsFor = APPLY_ON_ATTACKER,
    .onCritFor = Impl<ABILITY_DROIDEKA>.onCritFor,
    .breakable = TRUE,
    .negatesBurnAtkDrop = TRUE,
    .noBurnDamage = TRUE,
};
...
template <>
constexpr Ability Impl<ABILITY_BEAST_OF_GLUTTONY> = {
    // Craving + Gluttony + Resilience + Prismatic Fur
    .onEndTurn = Impl<ABILITY_CRAVING>.onEndTurn,
    .onDefender = Impl<ABILITY_RESILIENCE>.onDefender,
    .onDefensiveMultiplier = Impl<ABILITY_PRISMATIC_FUR>.onDefensiveMultiplier,
    .onBeforeAttack = Impl<ABILITY_PRISMATIC_FUR>.onBeforeAttack,
    .onBeforeAttackFor = Impl<ABILITY_PRISMATIC_FUR>.onBeforeAttackFor,
    .omniStab = TRUE,
};
