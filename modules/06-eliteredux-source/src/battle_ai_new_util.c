...
#define LOOKUP_PERCENTILE                                                                                                                                     \
    {TABLE_MATH(.01), TABLE_MATH(.02), TABLE_MATH(.03), TABLE_MATH(.04), TABLE_MATH(.05), TABLE_MATH(.06), TABLE_MATH(.07), TABLE_MATH(.08), TABLE_MATH(.09), \
     TABLE_MATH(.10), TABLE_MATH(.11), TABLE_MATH(.12), TABLE_MATH(.13), TABLE_MATH(.14), TABLE_MATH(.15), TABLE_MATH(.16), TABLE_MATH(.17), TABLE_MATH(.18), \
     TABLE_MATH(.19), TABLE_MATH(.20), TABLE_MATH(.21), TABLE_MATH(.22), TABLE_MATH(.23), TABLE_MATH(.24), TABLE_MATH(.25), TABLE_MATH(.26), TABLE_MATH(.27), \
     TABLE_MATH(.28), TABLE_MATH(.29), TABLE_MATH(.30), TABLE_MATH(.31), TABLE_MATH(.32), TABLE_MATH(.33), TABLE_MATH(.34), TABLE_MATH(.35), TABLE_MATH(.36), \
     TABLE_MATH(.37), TABLE_MATH(.38), TABLE_MATH(.39), TABLE_MATH(.40), TABLE_MATH(.41), TABLE_MATH(.42), TABLE_MATH(.43), TABLE_MATH(.44), TABLE_MATH(.45), \
     TABLE_MATH(.46), TABLE_MATH(.47), TABLE_MATH(.48), TABLE_MATH(.49), TABLE_MATH(.50), TABLE_MATH(.51), TABLE_MATH(.52), TABLE_MATH(.53), TABLE_MATH(.54), \
     TABLE_MATH(.55), TABLE_MATH(.56), TABLE_MATH(.57), TABLE_MATH(.58), TABLE_MATH(.59), TABLE_MATH(.60), TABLE_MATH(.61), TABLE_MATH(.62), TABLE_MATH(.63), \
     TABLE_MATH(.64), TABLE_MATH(.65), TABLE_MATH(.66), TABLE_MATH(.67), TABLE_MATH(.68), TABLE_MATH(.69), TABLE_MATH(.70), TABLE_MATH(.71), TABLE_MATH(.72), \
     TABLE_MATH(.73), TABLE_MATH(.74), TABLE_MATH(.75), TABLE_MATH(.76), TABLE_MATH(.77), TABLE_MATH(.78), TABLE_MATH(.79), TABLE_MATH(.80), TABLE_MATH(.81), \
     TABLE_MATH(.82), TABLE_MATH(.83), TABLE_MATH(.84), TABLE_MATH(.85), TABLE_MATH(.86), TABLE_MATH(.87), TABLE_MATH(.88), TABLE_MATH(.89), TABLE_MATH(.90), \
     TABLE_MATH(.91), TABLE_MATH(.92), TABLE_MATH(.93), TABLE_MATH(.94), TABLE_MATH(.95), TABLE_MATH(.96), TABLE_MATH(.97), TABLE_MATH(.98), TABLE_MATH(.99)}

#define LOOKUP_TENS \
    {TABLE_MATH(.1), TABLE_MATH(.2), TABLE_MATH(.3), TABLE_MATH(.4), TABLE_MATH(.5), TABLE_MATH(.6), TABLE_MATH(.7), TABLE_MATH(.8), TABLE_MATH(.9)}
...
// Compiler complains incorrectly that this is variably modified at file scope, if you change the math please uncomment this to make sure your changes are
// correct STATIC_ASSERT(1000 * (COMPOUND_3(.3)) == 657, TestCompoundMath)
...
int CalculateCompoundOdds(int probability, struct MoveState* moveState) {
    ...
    if (p2 == 1)
        p = UQ_4_12_PERCENT(probability);
    else
        p = gCompoundOdds[p2 - 2][(probability / 10) - 1];
...
int AlwaysCancelled(int battlerAtk, int move, struct MoveContainer* moveContainer) {
    if (gVolatileStructs[battlerAtk].throatChopTimer && gBattleMoves[move].flags & FLAG_SOUND) return TRUE;

    if (GetAbilityState(battlerAtk, ABILITY_TRUANT) && !IS_MOVE_STATUS(move)) return TRUE;
...
int CheckCancelled(int battlerAtk, int battlerDef, int move, struct MoveState* moveState, struct AiData* aiData) {
    ...
    if (!AreSameSide(battlerAtk, battlerDef) && !gProcessingExtraAttacks && moveState->speedValue.priority > 6) {
...
int BelowHalfHp(int battler) { return gBattleMons[battler].hp <= gBattleMons[battler].maxHP / 2; }
