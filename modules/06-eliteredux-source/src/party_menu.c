...
static void DisplayPartyPokemonDataToTeachMove(u8, u8);
...
static void CursorCb_GiveMegaStone(u8);
static void CursorCb_GiveMegaStone2(u8);
static void CursorCb_GiveMegaStone3(u8);
...
static void InitPartyMenu(u8 menuType, u8 layout, u8 partyAction, bool8 keepCursorPos, u8 messageId, TaskFunc task, MainCallback callback) {
    ...
    if (sPartyMenuInternal == NULL) {
        ...
        if (layout != KEEP_PARTY_LAYOUT) gPartyMenu.layout = layout;

        for (i = 0; i < ARRAY_COUNT(sPartyMenuInternal->data); i++) sPartyMenuInternal->data[i] = 0;
        for (i = 0; i < ARRAY_COUNT(sPartyMenuInternal->windowId); i++) sPartyMenuInternal->windowId[i] = WINDOW_NONE;
...
static void CB2_InitPartyMenu(void) {
    while (TRUE) {
        if (MenuHelpers_CallLinkSomething() == TRUE || ShowPartyMenu() == TRUE || MenuHelpers_LinkSomething() == TRUE) break;
    }
}
...
static bool8 ShowPartyMenu(void) {
    switch (gMain.state) {
        ...
        case 4:
            ...
            break;
        case 5:
            if (!MenuHelpers_LinkSomething()) ResetTasks();
            gMain.state++;
            break;
        case 6:
        ...
        case 7:
            ...
            break;
        case 8:
            if (AllocPartyMenuBgGfx()) gMain.state++;
            break;
        case 9:
...
static bool8 AllocPartyMenuBg(void) {
    ...
    if (sPartyBgTilemapBuffer == NULL) return FALSE;
...
static void FreePartyPointers(void) {
    if (sPartyMenuInternal) Free(sPartyMenuInternal);
    if (sPartyBgTilemapBuffer) Free(sPartyBgTilemapBuffer);
    if (sPartyBgGfxTilemap) Free(sPartyBgGfxTilemap);
    if (sPartyMenuBoxes) Free(sPartyMenuBoxes);
...
static bool8 DisplayPartyPokemonDataForMoveTutorOrEvolutionItem(u8 slot) {
    ...
    if (gPartyMenu.action == PARTY_ACTION_MOVE_TUTOR) {
        ...
        if (gPartyMenu.action != PARTY_ACTION_USE_ITEM) return FALSE;
        ...
        switch (CheckIfItemIsTMHMOrEvolutionStone(item)) {
            ...
            case 2:  // Evolution stone
                if (!GetMonData(currentPokemon, MON_DATA_IS_EGG) &&
                    GetEvolutionTargetSpecies(currentPokemon, EVO_MODE_ITEM_CHECK, item, SPECIES_NONE) != SPECIES_NONE)
                    return FALSE;
...
static void DisplayPartyPokemonDataToTeachMove(u8 slot, u8 tutor) {
    switch (CanMonLearnTMTutor(&gPlayerParty[slot], tutor)) {
...
static void DisplayPartyPokemonDataForMultiBattle(u8 slot) {
    ...
    if (gMultiPartnerParty[actualSlot].species == SPECIES_NONE) {
        ...
        DisplayPartyPokemonGender(
            gMultiPartnerParty[actualSlot].gender, gMultiPartnerParty[actualSlot].species, gMultiPartnerParty[actualSlot].nickname, menuBox);
...
static u8 *GetPartyMenuBgTile(u16 tileId) { return &sPartyBgGfxTilemap[tileId << 5]; }
    ...
    if (gPartyMenu.menuType == PARTY_MENU_TYPE_MULTI_SHOWCASE && slot >= MULTI_PARTY_SIZE) {
        ...
        if (gMultiPartnerParty[actualSlot].species != SPECIES_NONE) {
            CreatePartyMonIconSpriteParameterized(
                gMultiPartnerParty[actualSlot].species, gMultiPartnerParty[actualSlot].personality, &sPartyMenuBoxes[slot], 0);
...
static u8 GetPartyBoxPaletteFlags(u8 slot, u8 animNum) {
    ...
    if (GetMonData(&gPlayerParty[slot], MON_DATA_HP) == 0) palFlags |= PARTY_PAL_FAINTED;
    if (PartyBoxPal_ParnterOrDisqualifiedInArena(slot) == TRUE) palFlags |= PARTY_PAL_MULTI_ALT;
    if (gPartyMenu.action == PARTY_ACTION_SWITCHING) palFlags |= PARTY_PAL_SWITCHING;
    if (gPartyMenu.action == PARTY_ACTION_SWITCH) {
        if (slot == gPartyMenu.slotId || slot == gPartyMenu.slotId2) palFlags |= PARTY_PAL_TO_SWITCH;
    }
    if (gPartyMenu.action == PARTY_ACTION_SOFTBOILED && slot == gPartyMenu.slotId) palFlags |= PARTY_PAL_TO_SOFTBOIL;

    return palFlags;
}

static bool8 PartyBoxPal_ParnterOrDisqualifiedInArena(u8 slot) {
    if (gPartyMenu.layout == PARTY_LAYOUT_MULTI && (slot == 1 || slot == 4 || slot == 5)) return TRUE;
...
static void Task_ClosePartyMenuAndSetCB2(u8 taskId) {
    if (!gPaletteFade.active) {
        if (gPartyMenu.menuType == PARTY_MENU_TYPE_IN_BATTLE) UpdatePartyToFieldOrder();
...
u8 GetCursorSelectionMonId(void) { return gPartyMenu.slotId; }

u8 GetPartyMenuType(void) { return gPartyMenu.menuType; }
    ...
    if (*slotPtr == PARTY_SIZE) {
        ...
        switch (gPartyMenu.action) {
            ...
            case PARTY_ACTION_USE_ITEM:
                if (IsSelectedMonNotEgg((u8 *)slotPtr)) {
                    if (gPartyMenu.menuType == PARTY_MENU_TYPE_IN_BATTLE) sPartyMenuInternal->exitCallback = CB2_SetUpExitToBattleScreen;
    ...
    switch (gPartyMenu.action) {
        ...
        default:
            ...
            if (DisplayCancelChooseMonYesNo(taskId) != TRUE) {
                if (!MenuHelpers_LinkSomething()) gSpecialVar_0x8004 = PARTY_SIZE + 1;
...
static bool8 DisplayCancelChooseMonYesNo(u8 taskId) {
    ...
    if (stringPtr == NULL) return FALSE;
...
static u16 PartyMenuButtonHandler(s8 *slotPtr) {
    ...
    if (JOY_NEW(START_BUTTON)) return 8;
    ...
    // Pressed Cancel
    if ((JOY_NEW(A_BUTTON)) && *slotPtr == PARTY_SIZE + 1) return 2;
        ...
        switch (movementDir) {
            ...
            case MENU_DIR_RIGHT:
                if (gPlayerPartyCount != 1 && *slotPtr % 2 == 0) {
                    if (*slotPtr + 1 < gPlayerPartyCount) (*slotPtr)++;
                    // else
                    //     *slotPtr = sPartyMenuInternal->lastSelectedSlot;
                }
                break;
            case MENU_DIR_LEFT:
                if (*slotPtr != 0 && *slotPtr != PARTY_SIZE && *slotPtr != PARTY_SIZE + 1) {
                    if (*slotPtr - 1 >= 0 && *slotPtr % 2 == 1) (*slotPtr)--;
                    // sPartyMenuInternal->lastSelectedSlot = *slotPtr;
                    // *slotPtr = 0;
                }
                break;
        ...
        case MENU_DIR_RIGHT:
            if (*slotPtr == 0) {
                if (sPartyMenuInternal->lastSelectedSlot == 3) {
                    if (GetMonData(&gPlayerParty[3], MON_DATA_SPECIES) != SPECIES_NONE) *slotPtr = 3;
                } else if (GetMonData(&gPlayerParty[2], MON_DATA_SPECIES) != SPECIES_NONE) {
                    *slotPtr = 2;
                }
            } else if (*slotPtr == 1) {
                if (sPartyMenuInternal->lastSelectedSlot == 5) {
                    if (GetMonData(&gPlayerParty[5], MON_DATA_SPECIES) != SPECIES_NONE) *slotPtr = 5;
...
static s8 GetNewSlotDoubleLayout(s8 slotId, s8 movementDir) {
    while (TRUE) {
        slotId += movementDir;
        if ((u8)slotId >= PARTY_SIZE) return -1;
        if (GetMonData(&gPlayerParty[slotId], MON_DATA_SPECIES) != SPECIES_NONE) return slotId;
    }
}
...
bool8 IsPartyMenuTextPrinterActive(void) { return FuncIsActiveTask(Task_PrintAndWaitForText); }
...
enum {
    MESSAGE_RECEIVEDITEM,
    MESSAGE_GIVEMEGASTONE,
};

static void DisplayTookHeldItemMessage(struct Pokemon *mon, u16 item, bool8 keepOpen, u8 message) {
    GetMonNickname(mon, gStringVar1);
    CopyItemName(item, gStringVar2);
    switch (message) {
...
static u8 TryTakeMonItem(struct Pokemon *mon) {
    ...
    if (AddBagItem(item, 1) == FALSE) return 1;
...
static void BufferBagFullCantTakeItemMessage(u16 itemUnused) { StringExpandPlaceholders(gStringVar4, gText_BagFullCouldNotRemoveItem); }
...
static void Task_PartyMenuModifyHP(u8 taskId) {
    ...
    if (tHPToAdd == 0 || tHP == 0 || tHP == tMaxHP) {
        ...
        if (tHP > tStartHP) ConvertIntToDecimalStringN(gStringVar2, tHP - tStartHP, STR_CONV_MODE_LEFT_ALIGN, 3);
...
u8 GetAilmentFromStatus(u32 status) {
    ...
    if (status & STATUS1_FROSTBITE) return AILMENT_FSB;
    if (status & STATUS1_BLEED) return AILMENT_BLD;
    return AILMENT_NONE;
}
...
u8 GetMonAilment(struct Pokemon *mon) {
    ...
    if (GetMonData(mon, MON_DATA_HP) == 0) return AILMENT_FNT;
    ailment = GetAilmentFromStatus(GetMonData(mon, MON_DATA_STATUS));
    if (ailment != AILMENT_NONE) return ailment;
...
static void SetPartyMonsAllowedInMinigame(void) {
    ...
    if (gPartyMenu.menuType == PARTY_MENU_TYPE_MINIGAME) {
        ...
        if (gSpecialVar_0x8005 == 0) {
            for (i = 0; i < gPlayerPartyCount; i++) *ptr += IsMonAllowedInPokemonJump(&gPlayerParty[i]) << i;
        } else {
            for (i = 0; i < gPlayerPartyCount; i++) *ptr += IsMonAllowedInDodrioBerryPicking(&gPlayerParty[i]) << i;
        }
    }
}

static bool16 IsMonAllowedInPokemonJump(struct Pokemon *mon) {
    if (GetMonData(mon, MON_DATA_IS_EGG) != TRUE && IsSpeciesAllowedInPokemonJump(GetMonData(mon, MON_DATA_SPECIES))) return TRUE;
    return FALSE;
}

static bool16 IsMonAllowedInDodrioBerryPicking(struct Pokemon *mon) {
    if (GetMonData(mon, MON_DATA_IS_EGG) != TRUE && GetMonData(mon, MON_DATA_SPECIES) == SPECIES_DODRIO) return TRUE;
    return FALSE;
}

static bool8 IsMonAllowedInMinigame(u8 slot) {
    if (!((gPartyMenu.data1 >> slot) & 1)) return FALSE;
    return TRUE;
}
...
u8 CanMonLearnTMTutor(struct Pokemon *mon, u8 tutor) {
    u16 move;

    if (GetMonData(mon, MON_DATA_IS_EGG)) return CANNOT_LEARN_MOVE_IS_EGG;
...
static void InitPartyMenuWindows(u8 layout) {
    ...
    switch (layout) {
        case PARTY_LAYOUT_SINGLE:
            INIT_WINDOWS(sSinglePartyMenuWindowTemplate_Equal);  // sSinglePartyMenuWindowTemplate
            break;
        case PARTY_LAYOUT_DOUBLE:
            INIT_WINDOWS(sDoublePartyMenuWindowTemplate);
            break;
        case PARTY_LAYOUT_MULTI:
            INIT_WINDOWS(sMultiPartyMenuWindowTemplate);
            break;
        default:  // PARTY_LAYOUT_MULTI_SHOWCASE
            INIT_WINDOWS(sShowcaseMultiPartyMenuWindowTemplate);
            break;
    }
    DeactivateAllTextPrinters();
    for (i = 0; i < PARTY_SIZE; i++) FillWindowPixelBuffer(i, PIXEL_FILL(0));
...
static u16 *GetPartyMenuPalBufferPtr(u8 paletteId) { return &sPartyMenuInternal->palBuffer[paletteId]; }
...
static void BlitBitmapToPartyWindow(u8 windowId, const u8 *b, u8 c, u8 x, u8 y, u8 width, u8 height) {
    ...
    if (pixels != NULL) {
        for (i = 0; i < height; i++) {
            for (j = 0; j < width; j++) CpuCopy16(GetPartyMenuBgTile(b[x + j + ((y + i) * c)]), &pixels[(i * width + j) * 32], 32);
        }
...
static void DrawEmptySlot_Equal(u8 windowId) { BlitBitmapToPartyWindow(windowId, sEqualEmptySlotTileNums, 14, 0, 0, 14, 5); }  //
    ...
    if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE) {
        if (c != 0)
            menuBox->infoRects->blitFunc(menuBox->windowId,
                                         ...
                                         (menuBox->infoRects->dimensions[5] >> 3),
                                         (menuBox->infoRects->dimensions[6] >> 3) + 8,
                                         (menuBox->infoRects->dimensions[7] >> 3) + 4,
                                         FALSE);
        if (c != 2) DisplayPartyPokemonLevel(GetMonData(mon, MON_DATA_LEVEL), menuBox);
    }
}
    ...
    if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE) {
        if (c != 0)
            menuBox->infoRects->blitFunc(menuBox->windowId,
                                         ...
                                         FALSE);
        if (c != 2) DisplayPartyPokemonHP(GetMonData(mon, MON_DATA_HP), menuBox);
    }
}
    ...
    if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE) {
        if (c != 0)
            menuBox->infoRects->blitFunc(menuBox->windowId,
                                         ...
                                         FALSE);
        if (c != 2) DisplayPartyPokemonMaxHP(GetMonData(mon, MON_DATA_MAX_HP), menuBox);
    }
}
...
static void DisplayPartyPokemonHPBarCheck(struct Pokemon *mon, struct PartyMenuBox *menuBox) {
    if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE) DisplayPartyPokemonHPBar(GetMonData(mon, MON_DATA_HP), GetMonData(mon, MON_DATA_MAX_HP), menuBox);
}
...
static void DisplayPartyPokemonDescriptionText(u8 stringID, struct PartyMenuBox *menuBox, u8 c) {
    ...
    if (c != 2)
        AddTextPrinterParameterized3(
            menuBox->windowId, 1, menuBox->infoRects->descTextLeft, menuBox->infoRects->descTextTop, sFontColorTable[0], 0, sDescriptionStringTable[stringID]);
}
...
void DisplayPartyMenuStdMessage(u32 stringId) {
    ...
    if (*windowPtr != WINDOW_NONE) PartyMenuRemoveWindow(windowPtr);
...
static bool8 ShouldUseChooseMonText(void) {
    ...
    if (gPartyMenu.action == PARTY_ACTION_SEND_OUT) return TRUE;
...
static u8 DisplaySelectionWindow(u8 windowType) {
    ...
    if (windowType == SELECTWINDOW_MOVES) return sPartyMenuInternal->windowId[0];
    ...
    for (i = 0; i < sPartyMenuInternal->numActions; i++) {
        ...
        //     fontColorsId = 4;
        if (sPartyMenuInternal->actions[i] == MENU_SUB_FIELD_MOVES || sPartyMenuInternal->actions[i] == MENU_SUB_EVOLUTION ||
            sPartyMenuInternal->actions[i] == MENU_SUB_FORM_CHANGE || sPartyMenuInternal->actions[i] == MENU_SUB_LEVEL_UP ||
        ...
        if (sPartyMenuInternal->actions[i] == MENU_MEGA_STONE || sPartyMenuInternal->actions[i] == MENU_MEGA_STONE_2 ||
            sPartyMenuInternal->actions[i] == MENU_MEGA_STONE_3) {
...
static void PartyMenuDisplayYesNoMenu(void) { CreateYesNoMenu(&sPartyMenuYesNoWindowTemplate, 0x4F, 13, 0); }
...
static void SetPartyMonSelectionActions(struct Pokemon *mons, u8 slotId, u8 action) {
    ...
    switch (action) {
        ...
        default:
            ...
            for (i = 0; i < sPartyMenuInternal->numActions; i++) sPartyMenuInternal->actions[i] = sPartyMenuActions[action][i];
            break;
    }
}
...
static void SetPartyMonHeldItemSelectionActions(struct Pokemon *mons, u8 slotId) {
    ...
    // Give Mega Stone
    if (megaStones[0] && CheckBagHasItem(megaStones[0], 1) && helditem != megaStones[0])
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, MENU_MEGA_STONE);
...
static void CursorCb_Evolution(u8 taskId) {
    ...
    u16 targetSpecies = GetEvolutionForMon(&gPlayerParty[gPartyMenu.slotId], evolutionNum);

    PlaySE(SE_SELECT);
    if (targetSpecies != SPECIES_NONE) {
        gPartyMenu.exitCallback = CB2_ReturnToPartyMenuFromFlyMap;
        PartyMenuTryEvolutionFromSubMenu(taskId);
    } else {
...
static void CursorCb_FormChange(u8 taskId) {
    ...
    if (targetSpecies != SPECIES_NONE || TRUE) {
        ...
        DisplayPartyMenuMessage(gText_Space2, FALSE);
        ScheduleBgCopyTilemapToVram(2);
        gTasks[taskId].func = Task_ReturnToChooseMonAfterText;
    }
}
...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId) {
    ...
    if (!InBattlePike()) {
        ...
        if (GetMonData(&mons[slotId], MON_DATA_SPECIES) && allowChanges && !DisableLearnMoveActions &&
            (GetNumberOfRelearnableMoves(&mons[slotId]) || GetNumberOfEggMoves(&mons[slotId]) || GetNumberOfTMMoves(&mons[slotId]) ||
        ...
        // Form change
        if (allowChanges && FlagGet(FLAG_BADGE02_GET)) {
            for (i = 0; gFormChangeTable[species][i].method; i++) {
                targetSpecies = GetFormChangeForMon(&mons[slotId], i);
        ...
        // Level Up
        if (level < levelCap && allowChanges) AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, MENU_SUB_LEVEL_UP);
    }

    if (allowChanges) AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, MENU_SUB_FIELD_MOVES);
...
void CB2_ReturnToPartyMenuFromSummaryScreen(void) {
    ...
    InitPartyMenu(
        gPartyMenu.menuType, KEEP_PARTY_LAYOUT, gPartyMenu.action, TRUE, PARTY_MSG_DO_WHAT_WITH_MON, Task_TryCreateSelectionWindow, gPartyMenu.exitCallback);
}
...
static void SlidePartyMenuBoxSpritesOneStep(u8 taskId) {
    ...
    if (tSlot1SlideDir != 0) MovePartyMenuBoxSprites(&sPartyMenuBoxes[gPartyMenu.slotId], tSlot1SlideDir);
    if (tSlot2SlideDir != 0) MovePartyMenuBoxSprites(&sPartyMenuBoxes[gPartyMenu.slotId2], tSlot2SlideDir);
}

static void SlidePartyMenuBoxOneStep(u8 taskId) {
    s16 *data = gTasks[taskId].data;

    if (tSlot1SlideDir != 0) MoveAndBufferPartySlot(sSlot1TilemapBuffer, tSlot1Left + tSlot1Offset, tSlot1Top, tSlot1Width, tSlot1Height, tSlot1SlideDir);
    if (tSlot2SlideDir != 0) MoveAndBufferPartySlot(sSlot2TilemapBuffer, tSlot2Left + tSlot2Offset, tSlot2Top, tSlot2Width, tSlot2Height, tSlot2SlideDir);
    ScheduleBgCopyTilemapToVram(0);
...
static void Task_SlideSelectedSlotsOnscreen(u8 taskId) {
    ...
    // Continue sliding
        ...
        tSlot1Offset += tSlot1SlideDir;
        tSlot2Offset += tSlot2SlideDir;
        if (tSlot1Offset == 0) tSlot1SlideDir = 0;
        if (tSlot2Offset == 0) tSlot2SlideDir = 0;
    }
}
...
static void GiveMegaStone(int taskId, int itemCount) {
    ...
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[1]);
    ScheduleBgCopyTilemapToVram(2);
    DisplayTookHeldItemMessage(&gPlayerParty[gPartyMenu.slotId], megaEvoItem, TRUE, MESSAGE_GIVEMEGASTONE);
    gTasks[taskId].func = Task_UpdateHeldItemSprite;
}

static void CursorCb_GiveMegaStone(u8 taskId) { GiveMegaStone(taskId, 0); }

static void CursorCb_GiveMegaStone2(u8 taskId) { GiveMegaStone(taskId, 1); }

static void CursorCb_GiveMegaStone3(u8 taskId) { GiveMegaStone(taskId, 2); }
...
static void CursorCb_TakeItem(u8 taskId) {
    ...
    switch (TryTakeMonItem(mon)) {
        ...
        case 1:  // No room to take item
            ...
            break;
        default:  // Took item
            DisplayTookHeldItemMessage(mon, item, TRUE, MESSAGE_RECEIVEDITEM);
            break;
    }
    ScheduleBgCopyTilemapToVram(2);
    gTasks[taskId].func = Task_UpdateHeldItemSprite;
...
static void CursorCb_HeldItemSubMenu(u8 taskId) {
    PlaySE(SE_SELECT);
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[0]);
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[1]);
    SetPartyMonSelectionActions(gPlayerParty, gPartyMenu.slotId, ACTIONS_ITEMS_SUB);
    DisplaySelectionWindow(SELECTWINDOW_ACTIONS);
    DisplayPartyMenuStdMessage(PARTY_MSG_DO_WHAT_WITH_MON);
    gTasks[taskId].data[0] = 0xFF;
    gTasks[taskId].func = Task_HandleSelectionMenuInput;
...
static void CursorCb_EvolutionSubMenu(u8 taskId) {
    PlaySE(SE_SELECT);
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[0]);
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[1]);
...
static void CursorCb_NoEntry(u8 taskId) {
    ...
    for (i = 0; i < maxBattlers; i++) {
        if (gSelectedOrderFromParty[i] == (gPartyMenu.slotId + 1)) {
            for (j = i; j < (maxBattlers - 1); j++) gSelectedOrderFromParty[j] = gSelectedOrderFromParty[j + 1];
    ...
    for (i = 0; i < (maxBattlers - 1); i++) {
        if (gSelectedOrderFromParty[i] != 0) DisplayPartyPokemonDescriptionText(i + PARTYBOX_DESC_FIRST, &sPartyMenuBoxes[gSelectedOrderFromParty[i] - 1], 1);
    }
    DisplayPartyMenuStdMessage(PARTY_MSG_CHOOSE_MON);
    gTasks[taskId].func = Task_HandleChooseMonInput;
}
...
static void CursorCb_FieldMove(u8 taskId) {
    ...
    PlaySE(SE_SELECT);
    if (sFieldMoveCursorCallbacks[fieldMove].fieldMoveFunc == NULL) return;
...
static u16 GetFieldMoveMonSpecies(void) { return GetMonData(&gPlayerParty[gPartyMenu.slotId], MON_DATA_SPECIES); }

static void Task_CancelAfterAorBPress(u8 taskId) {
    if ((JOY_NEW(A_BUTTON)) || (JOY_NEW(B_BUTTON))) CursorCb_Cancel1(taskId);
}
...
static void UpdatePartyMonHPBar(u8 spriteId, struct Pokemon *mon) { UpdateHPBar(spriteId, GetMonData(mon, MON_DATA_HP), GetMonData(mon, MON_DATA_MAX_HP)); }
...
static void SpriteCB_UpdatePartyMonIcon(struct Sprite *sprite) { UpdateMonIconFrame(sprite); }
    ...
    switch (whichParty) {
        case TRADE_PLAYER:
            for (i = 0; i < partyCounts[TRADE_PLAYER]; i++) {
                ...
                if (item != ITEM_NONE) CreateHeldItemSpriteForTrade(partySpriteIds[i], FALSE);
            }
            break;
        case TRADE_PARTNER:
            for (i = 0; i < partyCounts[TRADE_PARTNER]; i++) {
                item = GetMonData(&gEnemyParty[i], MON_DATA_HELD_ITEM);
                if (item != ITEM_NONE) CreateHeldItemSpriteForTrade(partySpriteIds[i + PARTY_SIZE], FALSE);
            }
            break;
    }
}
...
static u8 CreateSmallPokeballButtonSprite(u8 x, u8 y) { return CreateSprite(&sSpriteTemplate_MenuPokeballSmall, x, y, 8); }

static void PartyMenuStartSpriteAnim(u8 spriteId, u8 animNum) { StartSpriteAnim(&gSprites[spriteId], animNum); }
...
static void SetPartyMonAilmentGfx(struct Pokemon *mon, struct PartyMenuBox *menuBox) { UpdatePartyMonAilmentGfx(GetMonAilment(mon), menuBox); }
...
// static void CB2_OpenTMCaseOnField(void) { InitTMCase(0, CB2_BagMenuFromStartMenu, 0); }
...
static void GetMedicineItemEffectMessage(u16 item, u32 statusCured) {
    switch (GetItemEffectType(item)) {
        ...
        case ITEM_EFFECT_CURE_FREEZE:
            if (statusCured & STATUS1_FREEZE) StringExpandPlaceholders(gStringVar4, gText_PkmnThawedOut);
            if (statusCured & STATUS1_FROSTBITE) StringExpandPlaceholders(gStringVar4, gText_PkmnFrostbiteHealed);
            break;
...
static bool8 NotUsingHPEVItemOnShedinja(struct Pokemon *mon, u16 item) {
    if (GetItemEffectType(item) == ITEM_EFFECT_HP_EV && GetMonData(mon, MON_DATA_SPECIES) == SPECIES_SHEDINJA) return FALSE;
    return TRUE;
}

static bool8 IsItemFlute(u16 item) {
    if (item == ITEM_BLUE_FLUTE || item == ITEM_RED_FLUTE || item == ITEM_YELLOW_FLUTE) return TRUE;
    return FALSE;
}
...
void ItemUseCB_Medicine(u8 taskId, TaskFunc task) {
    ...
    if (cannotUse != FALSE) {
        ...
        if (!IsItemFlute(item)) {
            PlaySE(SE_USE_ITEM);
            if (gPartyMenu.action != PARTY_ACTION_REUSABLE_ITEM) RemoveBagItem(item, 1);
        } else {
...
void Task_AbilityCapsule(u8 taskId) {
    ...
    switch (tState) {
        ...
        case 3:
            ...
            break;
        case 4:
            if (!IsPartyMenuTextPrinterActive()) tState++;
            break;
        case 5:
...
static void Task_ClosePartyMenuAfterText(u8 taskId) {
    if (IsPartyMenuTextPrinterActive() != TRUE) {
        if (gPartyMenuUseExitCallback == FALSE) sPartyMenuInternal->exitCallback = NULL;
        Task_ClosePartyMenu(taskId);
    }
}
...
static u16 ItemEffectToMonEv(struct Pokemon *mon, u8 effectType) {
    switch (effectType) {
        case ITEM_EFFECT_HP_EV:
            if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_SHEDINJA) return GetMonData(mon, MON_DATA_HP_EV);
            break;
        case ITEM_EFFECT_ATK_EV:
...
bool8 IsMoveHm(u16 move) {
    ...
    for (i = 0; i < NUM_HIDDEN_MACHINES; i++) {
        if (sTMHMMoves[i + NUM_TECHNICAL_MACHINES] == move) return TRUE;
    }
    return FALSE;
}
...
void ItemUseCB_TMHM(u8 taskId, TaskFunc task) { DisplayLearnMoveMessageAndClose(taskId, gText_PkmnCantLearnMove); }
...
static void Task_TryLearningNextMoveAfterText(u8 taskId) {
    if (IsPartyMenuTextPrinterActive() != TRUE) Task_TryLearningNextMove(taskId);
}
...
void ItemUseCB_CandyBox(u8 taskId, TaskFunc task) {
    ...
    if (level != MAX_LEVEL && (level < GetLevelCap())) {
        BufferMonStatsToTaskData(mon, arrayPtr);
        cannotUseEffect = ExecuteTableBasedItemEffect_(gPartyMenu.slotId, ITEM_RARE_CANDY, 0);
        BufferMonStatsToTaskData(mon, &ptr->data[NUM_STATS]);
    } else {
        PartyMenuTryEvolution(taskId);
        cannotUseEffect = TRUE;
...
void Task_ChangeMonForm(u8 taskId) {
    ...
    switch (tState) {
        ...
        case 5:
            ...
            if (tRemoveItem) RemoveBagItem(gSpecialVar_ItemId, 1);
            gTasks[taskId].func = Task_ClosePartyMenu;
            break;
    }
}
...
static void Task_TryItemUseFormChange(u8 taskId) {
    ...
    switch (gTasks[taskId].tState) {
        ...
        case 3:
            ...
            if (++gTasks[taskId].tAnimWait == 60) gTasks[taskId].tState++;
        ...
        case 5:
            ...
            break;
        case 6:
            if (!IsPartyMenuTextPrinterActive()) gTasks[taskId].tState++;
...
void ItemUseCB_FormChange(u8 taskId, TaskFunc task) { TryItemUseFormChange(taskId, task); }

void ItemUseCB_FormChange_ConsumedOnUse(u8 taskId, TaskFunc task) {
    if (TryItemUseFormChange(taskId, task)) RemoveBagItem(gSpecialVar_ItemId, 1);
}
...
u8 GetItemEffectType(u16 item) {
    ...
    if (!ITEM_HAS_EFFECT(item)) return ITEM_EFFECT_NONE;
    ...
    if (statusCure || (itemEffect[0] >> 7)) {
        if (statusCure == ITEM3_SLEEP)
            ...
            return ITEM_EFFECT_CURE_FREEZE;  // ITEM_EFFECT_CURE_FREEZE_FROSTBITE
...
void InitChooseHalfPartyForBattle(u8 unused) {
    ClearSelectedPartyOrder();
    InitPartyMenu(
        PARTY_MENU_TYPE_CHOOSE_HALF, PARTY_LAYOUT_SINGLE, PARTY_ACTION_CHOOSE_MON, FALSE, PARTY_MSG_CHOOSE_MON, Task_HandleChooseMonInput, gMain.savedCallback);
...
static u8 GetPartySlotEntryStatus(s8 slot) {
    if (GetBattleEntryEligibility(&gPlayerParty[slot]) == FALSE) return 2;
    if (HasPartySlotAlreadyBeenSelected(slot + 1) == TRUE) return 1;
    return 0;
}
...
static bool8 GetBattleEntryEligibility(struct Pokemon *mon) {
    ...
    if (GetMonData(mon, MON_DATA_IS_EGG) || GetMonData(mon, MON_DATA_LEVEL) > GetBattleEntryLevelCap() ||
        (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(BATTLE_FRONTIER_BATTLE_PYRAMID_LOBBY) &&
         gSaveBlock1Ptr->location.mapNum == MAP_NUM(BATTLE_FRONTIER_BATTLE_PYRAMID_LOBBY) && GetMonData(mon, MON_DATA_HELD_ITEM) != ITEM_NONE)) {
        return FALSE;
    }

    switch (VarGet(VAR_FRONTIER_FACILITY)) {
        case FACILITY_MULTI_OR_EREADER:
            if (GetMonData(mon, MON_DATA_HP) != 0) return TRUE;
            return FALSE;
        ...
        default:  // Battle Frontier
            ...
            for (; gFrontierBannedSpecies[i] != 0xFFFF; i++) {
                if (gFrontierBannedSpecies[i] == species) return FALSE;
            }
            return TRUE;
    }
}
...
static u8 CheckBattleEntriesAndGetMessage(void) {
    ...
    if (order[minBattlers - 1] == 0) {
        if (minBattlers == 1) return PARTY_MSG_NO_MON_FOR_BATTLE;
        ConvertIntToDecimalStringN(gStringVar1, minBattlers, STR_CONV_MODE_LEFT_ALIGN, 1);
        return PARTY_MSG_X_MONS_ARE_NEEDED;
    }

    facility = VarGet(VAR_FRONTIER_FACILITY);
    if (facility == FACILITY_UNION_ROOM || facility == FACILITY_MULTI_OR_EREADER) return 0xFF;
    ...
    for (i = 0; i < maxBattlers - 1; i++) {
        ...
        for (j = i + 1; j < maxBattlers; j++) {
            ...
            if (item != ITEM_NONE && item == GetMonData(&party[order[j] - 1], MON_DATA_HELD_ITEM)) return PARTY_MSG_NO_SAME_HOLD_ITEMS;
        }
    }
...
static bool8 HasPartySlotAlreadyBeenSelected(u8 slot) {
    ...
    for (i = 0; i < ARRAY_COUNT(gSelectedOrderFromParty); i++) {
        if (gSelectedOrderFromParty[i] == slot) return TRUE;
    }
    return FALSE;
}
...
static u8 GetBattleEntryLevelCap(void) {
    switch (VarGet(VAR_FRONTIER_FACILITY)) {
        ...
        case FACILITY_UNION_ROOM:
            return 30;
        default:  // Battle Frontier
            if (gSpecialVar_0x8004 == FRONTIER_LVL_50) return 50;
            return MAX_LEVEL;
    }
}
...
static u8 GetPartyLayoutFromBattleType(void) {
    if (IsDoubleBattle() == FALSE) return PARTY_LAYOUT_SINGLE;
    if (IsMultiBattle() == TRUE) return PARTY_LAYOUT_MULTI;
...
static u8 GetPartyMenuActionsTypeInBattle(struct Pokemon *mon) {
    if (GetMonData(&gPlayerParty[1], MON_DATA_SPECIES) != SPECIES_NONE && GetMonData(mon, MON_DATA_IS_EGG) == FALSE) {
        if (gPartyMenu.action == PARTY_ACTION_SEND_OUT) return ACTIONS_SEND_OUT;
        if (!(gBattleTypeFlags & BATTLE_TYPE_ARENA)) return ACTIONS_SHIFT;
    }
...
void BufferBattlePartyCurrentOrder(void) { BufferBattlePartyOrder(gBattlePartyCurrentOrder, GetPlayerFlankId()); }
...
static void BufferBattlePartyOrder(u8 *partyBattleOrder, u8 flankId) {
    ...
    for (i = 0; i < (int)ARRAY_COUNT(gBattlePartyCurrentOrder); i++) partyBattleOrder[i] = (partyIds[0 + (i * 2)] << 4) | partyIds[1 + (i * 2)];
}
...
static void BufferBattlePartyOrderBySide(u8 *partyBattleOrder, u8 flankId, u8 battlerId) {
    ...
    for (i = 0; i < 3; i++) partyBattleOrder[i] = (partyIndexes[0 + (i * 2)] << 4) | partyIndexes[1 + (i * 2)];
}
...
static void UpdatePartyToBattleOrder(void) {
    ...
    for (i = 0; i < PARTY_SIZE; i++) memcpy(&gPlayerParty[GetPartyIdFromBattlePartyId(i)], &partyBuffer[i], sizeof(struct Pokemon));
    Free(partyBuffer);
}
...
static void UpdatePartyToFieldOrder(void) {
    ...
    memcpy(partyBuffer, gPlayerParty, sizeof(gPlayerParty));
    for (i = 0; i < PARTY_SIZE; i++) memcpy(&gPlayerParty[GetPartyIdFromBattleSlot(i)], &partyBuffer[i], sizeof(struct Pokemon));
    Free(partyBuffer);
}
...
static void CB2_SetUpExitToBattleScreen(void) { SetMainCallback2(CB2_SetUpReshowBattleScreenAfterMenu); }
    ...
    if (!gPaletteFade.active) {
        ...
        if (tXPos == 0) {
            for (i = MULTI_PARTY_SIZE; i < PARTY_SIZE; i++) {
                if (gMultiPartnerParty[i - MULTI_PARTY_SIZE].species != SPECIES_NONE) AnimateSelectedPartyIcon(sPartyMenuBoxes[i].monSpriteId, 0);
            }
...
static void Task_WaitAfterMultiPartnerPartySlideIn(u8 taskId) {
    ...
    if (++data[0] == 256) Task_ClosePartyMenu(taskId);
}

static void MoveMultiPartyMenuBoxSprite(u8 spriteId, s16 x) {
    if (x >= 0) gSprites[spriteId].x2 = x;
}
...
void ChooseMonForDaycare(void) {
    InitPartyMenu(
        PARTY_MENU_TYPE_DAYCARE, PARTY_LAYOUT_SINGLE, PARTY_ACTION_CHOOSE_MON, FALSE, PARTY_MSG_CHOOSE_MON_2, Task_HandleChooseMonInput, BufferMonSelection);
}
...
static void BufferMonSelection(void) {
    ...
    if (gSpecialVar_0x8004 >= PARTY_SIZE) gSpecialVar_0x8004 = 0xFF;
    gFieldCallback2 = CB2_FadeFromPartyMenu;
...
static void CB2_ChooseContestMon(void) {
    ...
    if (gContestMonPartyIndex >= PARTY_SIZE) gContestMonPartyIndex = 0xFF;
...
void GetNumMovesSelectedMonHas(void) {
    ...
    for (i = 0; i < MAX_MON_MOVES; i++) {
        if (GetMonData(&gPlayerParty[gSpecialVar_0x8004], MON_DATA_MOVE1 + i) != MOVE_NONE) gSpecialVar_Result++;
    }
}
...
void MoveDeleterForgetMove(void) {
    ...
    for (i = gSpecialVar_0x8005; i < MAX_MON_MOVES - 1; i++) ShiftMoveSlot(&gPlayerParty[gSpecialVar_0x8004], i, i + 1);
}
        ...
        for (i = 0; i < CalculatePlayerPartyCount(); i++) {
            if (i != gSpecialVar_0x8004) {
                for (j = 0; j < MAX_MON_MOVES; j++) {
                    if (GetMonData(&gPlayerParty[i], MON_DATA_MOVE1 + j) == MOVE_SURF) return;
                }
            }
        }
        if (AnyStorageMonWithMove(move) != TRUE) gSpecialVar_Result = TRUE;
    }
}
...
void CursorCb_MoveItemCallback(u8 taskId) {
    ...
    if (gPaletteFade.active || MenuHelpers_LinkSomething()) return;
