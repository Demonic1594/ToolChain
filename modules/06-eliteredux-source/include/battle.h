...
#define IS_BATTLER_OF_TYPE(battlerId, type) \
    ((gBattleMons[battlerId].type1 == type || gBattleMons[battlerId].type2 == type || gBattleMons[battlerId].type3 == type))
...
extern Status3 gStatuses3[MAX_BATTLERS_COUNT];
extern Status4 gStatuses4[MAX_BATTLERS_COUNT];
...
// Index 0 is the entry currently being processed; 1..MAX_QUEUED_EXTRA_ATTACKS is the pending queue.
// (Was MAX_BATTLERS_COUNT + 1: multiple abilities/innates queueing follow-ups in doubles could overflow it.)
#define MAX_QUEUED_EXTRA_ATTACKS 12
extern struct ExtraAttackActionStruct gQueuedExtraAttackData[MAX_QUEUED_EXTRA_ATTACKS + 1];
