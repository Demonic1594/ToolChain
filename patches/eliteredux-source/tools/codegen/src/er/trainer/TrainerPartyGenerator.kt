...
object TrainerPartyGenerator : Generator {
  ...
  private fun getNegativeNatureStat(nature: Nature): NatureStat =
    when (nature) {
  ...
  // Deterministic stand-in for List.hashCode(): protobuf Message.hashCode() mixes in
  // the Descriptor's identity hashCode, which differs between JVM runs, so the
  // __sParty_* symbol names (and therefore the file bytes, and even linker section
  // ordering) were not reproducible. String.hashCode() is fixed by the Java spec and
  // protobuf's TextFormat toString() prints fields in descriptor order, so this is
  // stable across JVMs and generator batch configurations for identical party data.
  private fun partyHash(party: List<TrainerMon>): UInt =
    party.joinToString("\u0000") { it.toString() }.hashCode().toUInt()

  fun TrainerMon.validate(isHell: Boolean) = buildList {
    if (nonstandard.isNotEmpty()) return@buildList

    val possibleMoves = findLearnsetForSpecies(SPECIES_MAP[species]!!)
    val invalidMoves =
      moveList subtract
        (expandLearnset(possibleMoves, SPECIES_MAP[species]!!) +
            possibleMoves.levelList.flatMap { it.moveList })
          .toSet()
    addIfNot(isHell || invalidMoves.isEmpty()) { "Invalid moves: $invalidMoves" }

    val evTotal =
      ((hpEv / 4) + (atkEv / 4) + (defEv / 4) + (spatkEv / 4) + (spdefEv / 4) + (speEv / 4))

    addIfNot(if (isHell) evTotal >= 508 / 4 else evTotal == 508 / 4) { "Invalid EV total ${evTotal * 4}" }
    ...
    if (ironPill) {
      addIfNot(getNegativeNatureStat(nature) == SPE) {
        "Iron pill should use negative speed nature"
      }
    }
  }
  ...
  override fun generate(writer: OutputStreamWriter) {
    ...
    val partiesErrorLookup =
      TRAINERS_LIST.flatMap {
          listOf(it.ace.monList, it.elite.monList, it.hell.monList).map { list -> list to it }
        }
        .distinctBy { it.first }
        .toMap()

    val partyDifficulties =
      partiesErrorLookup.mapValues {
        when {
          it.value.ace.monList == it.key -> Difficulty.ACE
          it.value.elite.monList == it.key -> Difficulty.ELITE
          else -> Difficulty.HELL
        }
      }

    fun trainerForParty(party: List<TrainerMon>) =
      "${partiesErrorLookup[party]!!.id} ${partyDifficulties[party]}"

    val errors = buildList {
      for (party in allParties) {
        writer.appendLine(
          "static const struct TrainerMonItemCustomMoves __sParty_${partyHash(party)}[] = {"
        )
        for (mon in party) {
          with(mon) {
            addAll(
              validate(partyDifficulties[party] == Difficulty.HELL).map {
                "$it for mon ${mon.species}, party ${trainerForParty(party)}"
              }
            )
            ...
            val abilityIndex =
                SPECIES_MAP[species]!!.abilityList.indexOf(ability).also {
                    addIfNot(it > -1) {
                        "Mon $species does not have ability $ability, party: ${trainerForParty(party)}"
                    }
                }
      ...
      for (trainer in TRAINERS_LIST) {
        with(trainer) {
          ...
          writer.appendLine(
            ...
            |$IND$IND.party = __sParty_${partyHash(ace.monList)},
            |$IND$IND.partySizeInsane = ${actualElite.size},
            |$IND$IND.partyInsane = __sParty_${partyHash(actualElite)},
            |$IND$IND.partySizeHell = ${actualHell.size},
            |$IND$IND.partyHell = __sParty_${partyHash(actualHell)},
